...
function initFirebase() {
  try {
    // firebase.initializeApp/firestore/storage ya no son el SDK real de
    // Firebase — son el reemplazo local respaldado en IndexedDB (ver
    // local-db.js). Todo lo demás de esta función funciona exactamente igual
    // que antes, sin cambios, porque el reemplazo copia la misma forma de
    // llamar a collection/doc/onSnapshot/etc.
...
function ensureVistaInformeDetalle() {
  ...
  div.innerHTML = `
    ...
    <div class="informe-form-bloque">
      <label class="rotulo-mini">Consumibles</label>
      <textarea id="inputConsumibles" rows="4" placeholder="Un consumible por línea — ej: Disco de desbaste de 4 ½''"></textarea>
    </div>
    ...
    <div class="informe-form-bloque">
      ...
      <div style="display:flex; gap:6px; flex-wrap:wrap; margin-top:6px;">
        <select id="selCategoriaAnexo" class="input-mini"><option value="diagrama">Diagrama</option><option value="plano">Plano</option><option value="protocolo">Protocolo</option></select>
        <input type="text" id="inputDescAnexo" class="input-mini" placeholder="Descripción (leyenda)" style="flex:1; min-width:140px;">
      </div>
    ...
    <div class="informe-form-bloque">
      <label class="rotulo-mini">Conclusiones por actividad</label>
      <p style="font-size:11.5px; color:var(--ink-dim); margin:0 0 8px;">Van todas las actividades del informe. Si no escribes nada, se arma sola con el avance y los pasos registrados.</p>
      <div id="conclusionesListWrap"></div>
    </div>
  ...
  ['inputObjetivoPrincipal', 'inputAdminCentinela', 'inputAdminSemiva', 'inputEncargadoNombre', 'inputHerramientas', 'inputConsumibles'].forEach((id) => {
    const campoMap = {
  ...
  document.getElementById('btnEditarComoDocumento').addEventListener('click', async () => {
    ...
    try { await abrirVistaInteractiva(state.informeActivo); }
    catch (e) { console.error(e); showToast(e.message || 'No se pudo abrir el editor'); }
...
function renderAnexosLista() {
  ...
  const filasGuardadas = anexos.map((a, i) => `
    <div style="display:flex; align-items:center; gap:8px; padding:6px 0; border-bottom:1px solid var(--line);">
      <a href="${a.url}" target="_blank" rel="noopener" style="flex:1; font-size:12.5px; color:var(--brand); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escBit(((ANEXOS_CATEGORIAS.find((c) => c.id === a.categoria) || ANEXOS_CATEGORIAS[0]).etiqueta) + ': ' + (a.descripcion || a.nombre))}</a>
...
// Conclusiones (mismo dato que "Editar como documento": informe.conclusiones[otNum]).
function renderConclusionesLista() {
  const wrap = document.getElementById('conclusionesListWrap');
  if (!wrap || !state.informeActivo) return;
  const ots = (state.informeActivo.otNums || []).map((n) => allOts().find((o) => String(o.otNum) === String(n))).filter(Boolean);
  const propias = state.informeActivo.conclusiones || {};
  ...
  wrap.querySelectorAll('[data-conclusion]').forEach((ta) => {
    guardadoAutomaticoCampo(ta, async () => {
      ...
      // Igual al texto automático (o vacío) = sigue siendo automática.
      if (!texto || texto === generarConclusionTextoOt(ot).trim()) delete mapa[ta.dataset.conclusion];
      else mapa[ta.dataset.conclusion] = texto;
...
async function subirAnexoInforme(file) {
  ...
  try {
    ...
    const selCat = document.getElementById('selCategoriaAnexo');
    const inpDesc = document.getElementById('inputDescAnexo');
    const anexos = (state.informeActivo.anexos || []).concat([{
      nombre: file.name, url, tipo: file.type,
      categoria: selCat ? selCat.value : 'diagrama',
  ...
  try {
    ...
    for (const row of preparativosFotoRows) {
      if (row.file) {
        ...
        subidas++;
        fotos.push({ url, descripcion: row.descripcion || '', fecha: row.fecha || new Date().toISOString().slice(0, 10) });
      } else if (row.previewUrl || (row.descripcion || '').trim()) {
        // Un espacio de foto vacío (creado en "Editar como documento") se
        // conserva con su texto; si no tiene ni foto ni texto, se descarta.
        fotos.push({ url: row.previewUrl || '', descripcion: row.descripcion || '', fecha: row.fecha });
      }
    }
...
function abrirInformeDetalle(id) {
  ...
  document.getElementById('inputConsumibles').value = inf.consumiblesTexto || '';
  document.getElementById('inputPreparativosTexto').value = (inf.preparativosTexto || []).join('\n');
  preparativosFotoRows = (inf.preparativosFotos || []).map((f) => ({ file: null, previewUrl: f.url, descripcion: f.descripcion, fecha: f.fecha }));
  renderPreparativosFotoRows();
...
// modoEditor: cada texto lleva al final una marca ⟦EDIT:...⟧ (la pantalla la
...
// Los dibujos armados a partir de HTML (torta, Curva S) y las fotos ya
// procesadas se guardan en memoria: el editor regenera el Word en cada cambio
// y rehacerlos cada vez lo hacía lento. La clave incluye todo lo que influye
// en el resultado, así que un dato nuevo siempre da una imagen nueva.
const CACHE_IMAGENES_WORD = new Map();
async function conCacheImagenWord(clave, fn) {
  if (CACHE_IMAGENES_WORD.has(clave)) return CACHE_IMAGENES_WORD.get(clave);
  const res = await fn();
  if (res) {
    if (CACHE_IMAGENES_WORD.size > 60) CACHE_IMAGENES_WORD.delete(CACHE_IMAGENES_WORD.keys().next().value);
    CACHE_IMAGENES_WORD.set(clave, res);
  }
  return res;
}
async function htmlAPngCacheado(clave, cssText, innerHtml) {
  return conCacheImagenWord(clave, async () => {
    const wrap = document.createElement('div');
    wrap.style.cssText = cssText;
    wrap.innerHTML = innerHtml;
    document.body.appendChild(wrap);
    let canvas;
    try { canvas = await html2canvas(wrap, { scale: 2, backgroundColor: '#ffffff' }); } finally { wrap.remove(); }
...
function prepararFotoParaWord(url, cxDestino, cyDestino) {
  return conCacheImagenWord(`foto|${url}|${cxDestino}|${cyDestino}`, () => prepararFotoParaWordSinCache(url, cxDestino, cyDestino));
}
async function prepararFotoParaWordSinCache(url, cxDestino, cyDestino) {
  try {
...
// ---- Anexos: Diagramas / Planos / Protocolos. La plantilla trae DIAGRAMAS y
// PROTOCOLOS con ejemplos de otro informe (y no trae PLANOS) — todo eso se
// reemplaza por los anexos reales de este informe, cada uno con su imagen
// (sin recortar, ajustada a la hoja) y su leyenda numerada. ----
const ANEXOS_CATEGORIAS = [
  { id: 'diagrama', titulo: 'DIAGRAMAS', etiqueta: 'Diagrama', bookmark: { id: 16, name: '_Toc237061362' } },
  { id: 'plano', titulo: 'PLANOS', etiqueta: 'Plano', bookmark: null },
  { id: 'protocolo', titulo: 'PROTOCOLOS', etiqueta: 'Protocolo', bookmark: { id: 17, name: '_Toc237061363' } },
];

// ---- PDF → imágenes (pdf.js). El "worker" de pdf.js se arma como Blob para que
// funcione igual en el servidor, en el APK y abierto como archivo suelto. ----
let pdfWorkerListo = false;
async function prepararPdfJs() {
  if (typeof pdfjsLib === 'undefined') throw new Error('El lector de PDF no cargó');
  if (pdfWorkerListo) return;
  let codigo;
  if (window.__PDF_WORKER_B64) {
    const bin = atob(window.__PDF_WORKER_B64);
    codigo = new Uint8Array(bin.length);
    for (let i = 0; i < bin.length; i++) codigo[i] = bin.charCodeAt(i);
  } else {
    codigo = await (await fetch('./assets/vendor/pdf.worker.min.js')).arrayBuffer();
  }
  pdfjsLib.GlobalWorkerOptions.workerSrc = URL.createObjectURL(new Blob([codigo], { type: 'text/javascript' }));
  pdfWorkerListo = true;
}

// Devuelve { paginas: [{bytes, cx, cy}], total } (JPEG por página, ajustada sin
// recortar a maxCx × maxCy) o null si no se pudo leer el PDF.
function rasterizarPdfParaWord(url, maxCx, maxCy, maxPaginas = 12) {
  // Tope de tiempo: si el navegador tiene la pantalla en pausa (pestaña oculta)
  // pdf.js no dibuja; en vez de dejar el informe colgado se cae al aviso escrito.
  const conTope = (p) => Promise.race([p, new Promise((resolve) => setTimeout(() => resolve(null), 60000))]);
  return conCacheImagenWord(`pdf|${url}|${maxCx}|${maxCy}`, () => conTope((async () => {
    try {
      await prepararPdfJs();
      ...
      for (let n = 1; n <= Math.min(pdf.numPages, maxPaginas); n++) {
        ...
        let cx = maxCx, cy = cx / ratio;
        if (cy > maxCy) { cy = maxCy; cx = cy * ratio; }
        paginas.push({ bytes: new Uint8Array(await blob.arrayBuffer()), cx: Math.round(cx), cy: Math.round(cy) });
      }
      return { paginas, total: pdf.numPages };
    } catch (e) {
      console.error('No se pudo leer el PDF del anexo:', e);
      return null;
    }
  })()));
}

// Imágenes de "Distribución de población": la primera vive en el campo de
// siempre (distribucionPoblacionUrl, el del formulario clásico) y las demás en
// una lista aparte — así los dos formularios siguen viendo lo mismo.
function urlsDistribucionInforme(informe) {
  return [informe.distribucionPoblacionUrl, ...(informe.distribucionPoblacionUrls || [])].filter(Boolean);
}
async function guardarUrlsDistribucionInforme(lista) {
  await guardarCampoInformeActivo('distribucionPoblacionUrl', lista[0] || '');
  await guardarCampoInformeActivo('distribucionPoblacionUrls', lista.slice(1));
}

// Imagen de un anexo: se escala COMPLETA para que quepa en la hoja (nada de
// recortes: son planos y diagramas, se tiene que ver todo).
function prepararImagenAnexoParaWord(url, maxCx, maxCy) {
  return conCacheImagenWord(`anexo|${url}|${maxCx}|${maxCy}`, () => prepararImagenAnexoParaWordSinCache(url, maxCx, maxCy));
}
async function prepararImagenAnexoParaWordSinCache(url, maxCx, maxCy) {
  try {
    const r = await fetch(url);
    const blob = await r.blob();
    const bitmap = await createImageBitmap(blob);
    const escala = Math.min(maxCx / bitmap.width, maxCy / bitmap.height);
    ...
    return { bytes: new Uint8Array(await outBlob.arrayBuffer()), cx, cy };
  } catch (e) {
    console.error('No se pudo preparar la imagen del anexo:', e);
    return null;
  }
}

function buildTituloAnexoWord(cat, conSaltoDePagina) {
  const pid = randParaIdWord();
  const bmS = cat.bookmark ? `<w:bookmarkStart w:id="${cat.bookmark.id}" w:name="${cat.bookmark.name}"/>` : '';
  const bmE = cat.bookmark ? `<w:bookmarkEnd w:id="${cat.bookmark.id}"/>` : '';
...
function buildLeyendaAnexoWord(etiqueta, numero, texto, marcaEditor) {
  ...
  const rPrB = '<w:rPr><w:rFonts w:ascii="Century Gothic" w:hAnsi="Century Gothic"/><w:b/><w:bCs w:val="0"/></w:rPr>';
  return `<w:p w14:paraId="${pid}" w14:textId="${pid}"><w:pPr><w:pStyle w:val="Descripcin"/><w:jc w:val="center"/></w:pPr><w:r>${rPrB}<w:t xml:space="preserve">${etiqueta} ${numero}: </w:t></w:r><w:r>${rPr}<w:t xml:space="preserve">${escXmlWord(texto)}${marcaEditor ? ' ' + escXmlWord(marcaEditor) : ''}</w:t></w:r></w:p>`;
}

// Texto suelto que SOLO existe en el editor ("Editar como documento"): la
// pantalla lo reemplaza por un botón. El Word que se descarga no lo lleva.
function buildMarcaEditorWord(texto) {
  const pid = randParaIdWord();
  return `<w:p w14:paraId="${pid}" w14:textId="${pid}"><w:pPr><w:jc w:val="center"/></w:pPr><w:r><w:t>${escXmlWord(texto)}</w:t></w:r></w:p>`;
}
...
// Marca (solo en el editor) de "quitar esta fila de fotos": lleva las
// referencias a las fotos del par, para saber qué borrar.
function marcaQuitarParFotos(a, b) {
  const refs = [a.ref, b && b.ref].filter(Boolean).join('|');
  return ` ⟦EDIT:fechafila:${refs}⟧ ⟦X:fotos:${refs}⟧`;
}

// Todo lo que el editor necesita para un par: por cada espacio la marca de
// foto (añadir / quitar) y el texto editable. `grupoNuevo` identifica dónde
// se agrega una foto si el par tiene el segundo espacio libre.
function datosParEditor(a, b, grupoNuevo) {
  const marcaSlot = (f) => (f.hasFoto ? `⟦FOTO:del:${f.ref}⟧` : `⟦FOTO:add:${f.ref}⟧`);
  const desc = (f) => (f.descripcion && f.descripcion.trim() ? f.descripcion : '(toca aquí para escribir)') + ` ⟦EDIT:desc:${f.ref}⟧`;
  return {
    marcas: [marcaSlot(a), b ? marcaSlot(b) : `⟦FOTO:new:${grupoNuevo}⟧`],
    descA: desc(a),
    descB: b ? desc(b) : null,
  };
}

// Dos fotos forman un par solo si son de la misma fecha Y del mismo grupo (la
// misma entrada de bitácora): así una fila nueva nunca se junta con la foto
// suelta de otra.
function fotosSonPar(a, siguiente) {
  return !!siguiente && siguiente.fechaTexto === a.fechaTexto && siguiente.grupo === a.grupo;
}

function buildBloqueRegistroFotosWord(fotosEmbebidas, modoEditor) {
  let out = '';
  let contador = 0;
  let i = 0;
  while (i < fotosEmbebidas.length) {
    ...
    const ed = modoEditor ? datosParEditor(a, b, a.grupo) : null;
...
// Fila que solo existe en el editor: una celda con el texto que la pantalla
// convierte en el botón "Añadir fila de fotos".
function buildFilaMarcaEditorWord(texto) {
  const pid = randParaIdWord();
  return `<w:tr w14:paraId="${pid}" w14:textId="${pid}"><w:trPr><w:jc w:val="center"/></w:trPr><w:tc><w:tcPr><w:tcW w:w="10704" w:type="dxa"/><w:gridSpan w:val="2"/></w:tcPr>${buildMarcaEditorWord(texto)}</w:tc></w:tr>`;
}
...
function buildBloqueRegistroFotosPreparativosWord(fotosEmbebidas, modoEditor) {
  ...
  while (i < fotosEmbebidas.length) {
    ...
    const ed = modoEditor ? datosParEditor(a, b, 'p:' + a.ref.slice(2)) : null;
...
function generarConclusionTextoOt(ot) {
  ...
  const bullets = state.bitacora.filter((b) => String(b.otNum) === String(ot.otNum)).flatMap((b) => b.bullets || []).filter((b) => b && b.trim());
  if (!bullets.length) {
    return pct >= 100
...
// ---- Listas de la parte final: LISTA DE COMPONENTES / HERRAMIENTAS / CONSUMIBLES.
// Las tres son tablas de la plantilla con filas de EJEMPLO (de otro trabajo):
// se borran y se arman las filas reales clonando el formato de la primera fila
// de datos (la columna ITEM es numeración automática de Word). Si no hay datos
// queda solo el encabezado. ----
function reemplazarFilasListaWord(xml, bookmarkName, filas, conCantidad, marcaEditor) {
  const bm = xml.indexOf(`w:name="${bookmarkName}"`);
  if (bm === -1) return xml;
  const tIni = xml.indexOf('<w:tbl>', bm);
  const tFin = tIni === -1 ? -1 : xml.indexOf('</w:tbl>', tIni);
  if (tIni === -1 || tFin === -1) return xml;
  const tbl = xml.slice(tIni, tFin);
  if (tbl.indexOf('<w:tbl>', 7) !== -1) return xml; // tabla anidada: no se toca
  const trs = tbl.match(/<w:tr[ >][\s\S]*?<\/w:tr>/g) || [];
  if (trs.length < 2) return xml;
  const modelo = trs[1];
  const celdas = modelo.match(/<w:tc>[\s\S]*?<\/w:tc>/g) || [];
  if (celdas.length < (conCantidad ? 3 : 2)) return xml;
  const limpiar = (s) => s.replace(/ w14:(paraId|textId)="[^"]*"/g, '');
  const abre = limpiar(/^<w:tr[^>]*>/.exec(modelo)[0]);
  const trPr = (/<w:trPr>[\s\S]*?<\/w:trPr>/.exec(modelo) || [''])[0];
  const celdaTexto = (celda, texto) => {
    const tcPr = (/<w:tcPr>[\s\S]*?<\/w:tcPr>/.exec(celda) || [''])[0];
    const pPr = (/<w:pPr>[\s\S]*?<\/w:pPr>/.exec(celda) || [''])[0];
    const rPr = (/<w:r[ >](?:(?!<\/w:r>)[\s\S])*?(<w:rPr>[\s\S]*?<\/w:rPr>)/.exec(celda) || [])[1] || '';
    return `<w:tc>${tcPr}<w:p>${pPr}<w:r>${rPr}<w:t xml:space="preserve">${escXmlWord(texto)}</w:t></w:r></w:p></w:tc>`;
  };
  const filasXml = filas.map((f) => `${abre}${trPr}${limpiar(celdas[0])}${celdaTexto(celdas[1], f.desc)}${conCantidad ? celdaTexto(celdas[2], f.cant) : ''}</w:tr>`).join('');
  const finEncab = tIni + tbl.indexOf(trs[0]) + trs[0].length;
  const huerfanos = extraerBookmarkEndsHuerfanos(xml.slice(finEncab, tFin));
  return xml.slice(0, finEncab) + filasXml + huerfanos
    + xml.slice(tFin, tFin + '</w:tbl>'.length)
    + (marcaEditor ? buildMarcaEditorWord(marcaEditor) : '')
    + xml.slice(tFin + '</w:tbl>'.length);
}

// "Llave torquímetro 1" — 2 und." / "nombre<TAB>cantidad" (pegado de Excel) /
// "nombre" → { desc, cant }. Sin cantidad se pone 01, como la plantilla.
function parsearHerramientasTexto(texto) {
  return String(texto || '').split('\n').map((l) => l.trim()).filter(Boolean).map((l) => {
    ...
    if (l.includes('\t')) {
      ...
      const m = /^(.*?)\s*(?:—|–|-|:|x|×)\s*(\d+)\s*(?:und\.?|unid\.?|unidades?|u\.?)?$/i.exec(l);
      if (m && m[1]) { desc = m[1].trim(); cant = m[2]; }
    }
    const n = parseInt(cant, 10);
    return { desc, cant: Number.isFinite(n) && n > 0 ? String(n).padStart(2, '0') : '01' };
  });
}
...
function buildFilaRecomendacionWord(ot, texto, modoEditor) {
  const otTexto = ot.tipo === 'Emergente' ? 'EMERGENTE' : ot.otNum;
  return buildFilaItemTextoWord(RECOMENDACIONES_NUM_ID, [
    { text: `OT: ${otTexto} | ${tituloSinEquipo(ot.descripcion).toUpperCase()}` + (modoEditor ? ` ⟦EDIT:recotitulo:${ot.otNum}⟧ ⟦X:reco:${ot.otNum}⟧` : ''), bold: true },
    { text: modoEditor ? (texto || '(toca aquí para escribir la recomendación)') + ` ⟦EDIT:reco:${ot.otNum}⟧` : texto, bold: false },
  ]);
}
...
const SALTO_PAGINA_MINIMO_XML = '<w:p><w:pPr><w:spacing w:before="0" w:after="0" w:line="20" w:lineRule="exact"/></w:pPr><w:r><w:br w:type="page"/></w:r></w:p>';
function buildParrafoImagenCenteredWord(drawingXml, conSaltoDePagina) {
  const pid = randParaIdWord();
  return `<w:p w14:paraId="${pid}" w14:textId="${pid}"><w:pPr><w:jc w:val="center"/></w:pPr><w:r>${drawingXml}</w:r>${conSaltoDePagina ? '<w:r><w:br w:type="page"/></w:r>' : ''}</w:p>`;
}
...
async function generateInformeWordBlob(informe, opciones = {}) {
  // modoEditor: es el documento de la pantalla "Editar como documento" — lleva
  ...
  // ---- La segunda hoja (título + foto + cuadro ELABORADO/REVISADO/VALIDADO POR
  // con su fila FECHA) debe quedar entera en UNA hoja. En la plantilla esa hoja
  // queda justa, y con un título de 2 o más líneas la última fila (FECHA) se
  // iba a la hoja siguiente. Se libera espacio: se quita el párrafo vacío entre
  // la foto y el cuadro y la foto se achica un 6% (mismas proporciones). ----
  try {
    const portadaIdx = xml.indexOf(PORTADA_FOTO_ANCLA);
    const dIni = portadaIdx !== -1 ? xml.indexOf('<w:drawing', portadaIdx) : -1;
    if (dIni !== -1 && dIni - portadaIdx < 3000 && !/^<w:drawing[^>]*><wp:anchor/.test(xml.slice(dIni, dIni + 200))) {
      const dFin = xml.indexOf('</w:drawing>', dIni) + '</w:drawing>'.length;
      // Cuánto se achica la foto: 6% siempre, y algo más por cada línea que
      // el título ocupe de más (a 16pt entran ~34 letras por línea).
      const lineasTitulo = Math.max(1, Math.ceil(((informe.tituloGeneral || '').length || 1) / 34));
      const factor = Math.max(0.78, 0.94 - 0.06 * (lineasTitulo - 1));
      const escala = (n) => Math.round(parseInt(n, 10) * factor);
      const dibujo = xml.slice(dIni, dFin)
        .replace(/<wp:extent cx="(\d+)" cy="(\d+)"/, (m, cx, cy) => `<wp:extent cx="${escala(cx)}" cy="${escala(cy)}"`)
        .replace(/<a:ext cx="(\d+)" cy="(\d+)"/, (m, cx, cy) => `<a:ext cx="${escala(cx)}" cy="${escala(cy)}"`);
      xml = xml.slice(0, dIni) + dibujo + xml.slice(dFin);
      // La foto vive en una fila de la tabla contenedora con alto mínimo fijo
      // (12,5 cm): también se achica, si no la foto se achica pero la fila no.
      const filaIni = Math.max(xml.lastIndexOf('<w:tr ', dIni), xml.lastIndexOf('<w:tr>', dIni));
      let filaFin = xml.indexOf('</w:tr>', dIni) + '</w:tr>'.length;
      const filaFoto = xml.slice(filaIni, filaFin);
      const filaFotoNueva = filaFoto.replace(/<w:trHeight w:val="(\d+)"/, (m, h) => `<w:trHeight w:val="${Math.round(parseInt(h, 10) * factor)}"`);
      xml = xml.slice(0, filaIni) + filaFotoNueva + xml.slice(filaFin);
      filaFin = filaIni + filaFotoNueva.length;
      // Y se quita la fila vacía que separa la foto del cuadro de firmas.
      const filaSig = /^<w:tr[ >][\s\S]*?<\/w:tr>/.exec(xml.slice(filaFin));
      if (filaSig && !/<w:t[ >]|<w:drawing|<w:tbl>|<w:br |<w:bookmark|<w:sectPr/.test(filaSig[0])) {
        xml = xml.slice(0, filaFin) + xml.slice(filaFin + filaSig[0].length);
      }
    }
  } catch (e) {
    console.error('No se pudo ajustar la segunda hoja:', e);
  }
  ...
  // Puede haber más de una imagen (se agregan/quitan desde el editor). La
  // imagen de ejemplo de la plantilla (dotación de OTRO trabajo) nunca se
  // deja: si no hay imágenes propias, la tabla se saca (en el editor queda un
  // botón para agregar). Las imágenes se ajustan enteras, sin recortar. ----
  try {
    const urlsDistrib = urlsDistribucionInforme(informe);
    const distribAnclaIdx = xml.indexOf(DISTRIBUCION_POBLACION_ANCLA, xml.indexOf(DISTRIBUCION_POBLACION_ANCLA) + 1);
    const drawingStartIdx = distribAnclaIdx !== -1 ? xml.indexOf('<w:drawing', distribAnclaIdx) : -1;
    if (drawingStartIdx !== -1 && drawingStartIdx - distribAnclaIdx < 4000) {
      const tblIni = xml.lastIndexOf('<w:tbl>', drawingStartIdx);
      const tblFin = xml.indexOf('</w:tbl>', drawingStartIdx) + '</w:tbl>'.length;
      const tablaOriginal = xml.slice(tblIni, tblFin);
      const filasOriginal = tablaOriginal.match(/<w:tr[ >][\s\S]*?<\/w:tr>/g) || [];
      if (filasOriginal.length === 2 && tablaOriginal.indexOf('<w:tbl>', 7) === -1) {
        const extentMatch = xml.slice(drawingStartIdx, drawingStartIdx + 600).match(/<wp:extent cx="(\d+)" cy="(\d+)"/);
        const cxMax = extentMatch ? parseInt(extentMatch[1], 10) : Math.round(25 * INFORME_EMU_POR_CM);
        const cyMax = Math.round(11 * INFORME_EMU_POR_CM);
        const trPr = '<w:trPr><w:jc w:val="center"/></w:trPr>';
        const tcPr = '<w:tcPr><w:tcW w:w="14312" w:type="dxa"/><w:vAlign w:val="center"/></w:tcPr>';
        const parrafoTexto = (t) => `<w:p><w:pPr><w:jc w:val="center"/></w:pPr><w:r><w:t>${escXmlWord(t)}</w:t></w:r></w:p>`;
        let filasNuevas = '';
        for (const [i, url] of urlsDistrib.entries()) {
          const preparada = await prepararImagenAnexoParaWord(url, cxMax, cyMax);
          const marca = opciones.modoEditor ? parrafoTexto(`⟦X:distrib:${i}⟧`) : '';
          if (!preparada) { if (marca) filasNuevas += `<w:tr>${trPr}<w:tc>${tcPr}${marca}${parrafoTexto('(no se pudo leer esta imagen)')}</w:tc></w:tr>`; continue; }
          contadorImagen++;
          const nombreArchivo = `distribpoblacion_${Date.now()}_${contadorImagen}.jpeg`;
          const rId = `rId${nextRid++}`;
          zip.file(`word/media/${nombreArchivo}`, preparada.bytes);
          nuevasRelsXml += `<Relationship Id="${rId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/${nombreArchivo}"/>`;
          const drawing = buildDrawingXmlWord(rId, preparada.cx, preparada.cy, 820000 + contadorImagen);
          filasNuevas += `<w:tr>${trPr}<w:tc>${tcPr}${marca}<w:p><w:pPr><w:jc w:val="center"/></w:pPr><w:r>${drawing}</w:r></w:p></w:tc></w:tr>`;
        }
        if (opciones.modoEditor) filasNuevas += `<w:tr>${trPr}<w:tc>${tcPr}${parrafoTexto('⟦AÑADIR:distrib⟧')}</w:tc></w:tr>`;
        let tablaNueva = '';
        if (filasNuevas) {
          // Encabezado de la tabla (propiedades + grilla) tal cual la plantilla,
          // y la fila de leyenda original al final (solo si hay imágenes).
          const cabecera = tablaOriginal.slice(0, tablaOriginal.indexOf(filasOriginal[0]));
          const leyenda = urlsDistrib.length ? filasOriginal[1] : '';
          tablaNueva = cabecera + filasNuevas + leyenda + '</w:tbl>';
        }
        // Una tabla no puede quedar pegada a otra ni sin párrafo: la plantilla
        // ya trae un párrafo antes y otro después, así que se puede quitar entera.
        const huerfanos = extraerBookmarkEndsHuerfanos(tablaOriginal);
        xml = xml.slice(0, tblIni) + tablaNueva + huerfanos + xml.slice(tblFin);
      }
    }
  } catch (e) {
    console.error('No se pudo armar la imagen de distribución de población en el Word:', e);
  }
  ...
  try {
    ...
    // La plantilla trae, justo después de la introducción, una hoja de Excel
    // incrustada (imagen) con una tabla y una torta de EJEMPLO de otro informe
    // (OTs y % que no son los de este). Se saca junto con los párrafos vacíos
    // que la siguen — la tabla y la torta reales se arman más abajo.
    // La tabla y la torta reales van EXACTAMENTE donde estaba esa imagen: así
    // la hoja siguiente (administradores/supervisores + programa del servicio)
    // sigue empezando en hoja nueva, como en la plantilla.
    let puntoOle = -1;
    let saltoIncluido = false;
    if (cumplHeadingIdx !== -1) {
      const objIdx = xml.indexOf('<w:object ', cumplHeadingIdx);
      const tblIdx = xml.indexOf('<w:tbl>', cumplHeadingIdx);
      if (objIdx !== -1 && (tblIdx === -1 || objIdx < tblIdx)) {
        const pIni = xml.lastIndexOf('<w:p ', objIdx);
        const pFin = xml.indexOf('</w:p>', objIdx) + '</w:p>'.length;
        let fin = pFin;
        // Párrafo vacío = sin texto ni imagen (puede traer <w:pPr> con formato).
        const vacio = /^<w:p(?: [^>]*)?(?:\/>|><w:pPr>(?:(?!<\/w:pPr>)[\s\S])*<\/w:pPr><\/w:p>|><\/w:p>)/;
        let m;
        while ((m = vacio.exec(xml.slice(fin)))) fin += m[0].length;
        // El párrafo que solo trae el salto de página también se saca: el salto
        // se pone dentro del último párrafo del bloque nuevo (si quedara aparte y
        // la hoja está llena, caía en una hoja en blanco).
        const soloSalto = /^<w:p[ >](?:(?!<\/w:p>)[\s\S])*?<w:br w:type="page"\/>(?:(?!<\/w:p>)[\s\S])*?<\/w:p>/.exec(xml.slice(fin));
        if (soloSalto && !/<w:t[ >]|<w:drawing/.test(soloSalto[0])) { fin += soloSalto[0].length; saltoIncluido = true; }
        xml = xml.slice(0, pIni) + xml.slice(fin);
        puntoOle = pIni;
      }
    }
    const puntoInsercion = puntoOle !== -1 ? puntoOle : (cumplHeadingIdx !== -1 ? xml.indexOf('<w:tbl>', cumplHeadingIdx) : -1);
    if (puntoInsercion !== -1 && ots.length) {
      ...
      const imgTorta = await htmlAPngCacheado('pie|' + svgTorta, 'position:fixed; left:-10000px; top:0; width:560px; height:320px; background:#ffffff;', svgTorta);

      // Igual que la plantilla: la tabla y la torta van solas en su hoja
      // (salto antes y salto después), como iba la imagen de ejemplo.
      let bloqueXml = (saltoIncluido ? SALTO_PAGINA_MINIMO_XML : buildParrafoVacioWord()) + tablaXml + (saltoIncluido ? '' : buildParrafoVacioWord());
      if (imgTorta) {
        const bytesTorta = imgTorta.bytes;
        const cxTorta = Math.round(14 * INFORME_EMU_POR_CM);
        const cyTorta = Math.round(cxTorta * (imgTorta.h / imgTorta.w));
        contadorImagen++;
        ...
        bloqueXml += buildParrafoImagenCenteredWord(drawingTortaXml) + (saltoIncluido ? '' : buildParrafoVacioWord());
      }
      // Salto de página en un párrafo de 1 pt de alto: así, aunque la hoja
      // quede llena, el salto no arrastra una hoja en blanco.
      if (saltoIncluido) bloqueXml += SALTO_PAGINA_MINIMO_XML;

      xml = xml.slice(0, puntoInsercion) + bloqueXml + xml.slice(puntoInsercion);
    } else if (puntoInsercion !== -1 && saltoIncluido) {
      // Sin actividades no hay tabla ni torta, pero la hoja siguiente igual empieza en hoja nueva.
      xml = xml.slice(0, puntoInsercion) + SALTO_PAGINA_MINIMO_XML + xml.slice(puntoInsercion);
    }
  } catch (e) {
    ...
    if (prepFotosAnclaIdx !== -1 && (limiteSuperior === -1 || prepFotosAnclaIdx < limiteSuperior)) {
      ...
      if (primerImagenPrepIdx !== -1 && (limiteSuperior === -1 || primerImagenPrepIdx < limiteSuperior)) {
        ...
        for (const foto of prepFotos) {
          const preparada = foto.url ? await prepararFotoParaWord(foto.url) : null;
          // Un espacio sin foto ni texto solo se ve en el editor.
          if (!preparada && !opciones.modoEditor && !(foto.descripcion || '').trim()) continue;
          let drawingXml = '';
          if (preparada) {
            contadorImagen++;
          ...
          prepFotosEmbebidas.push({
            fechaTexto: foto.fecha ? fechaDDMMYYYY(foto.fecha) : fechaHoy,
            ref: `p:${prepFotos.indexOf(foto)}`,
            drawingXml,
            hasFoto: !!preparada,
        ...
        const nuevoBloqueFotos = (prepFotosEmbebidas.length ? buildBloqueRegistroFotosPreparativosWord(prepFotosEmbebidas, opciones.modoEditor) : '')
          + (opciones.modoEditor ? buildMarcaEditorWord('⟦AÑADIR:fotosprep⟧') : '');
  ...
  entradasTexto.forEach(({ ot, entry }) => {
    ...
    if (turnoKey !== lastTurnoKey) {
      ...
      if (fechaTexto && !esMismaFechaQueAnterior) {
        rowsXml += buildFilaFechaWord(fechaTexto + (opciones.modoEditor ? ` ⟦EDIT:fechadia:${entry.fecha}⟧ ⟦X:dia:${entry.fecha}⟧` : ''));
      }
  ...
  // fila al final que la pantalla convierte en el botón "Añadir día".
  if (opciones.modoEditor) rowsXml += buildFilaFechaWord('', '⟦AÑADIR:dia⟧');

  // El editor ("Editar como documento") se abre aunque todavía no haya comentarios: ahí justo se está armando el documento. Para descargar el Word final sí se exige.
  if (!rowsXml && !opciones.permitirSinComentarios) throw new Error('Esta actividad todavía no tiene comentarios cargados — agrega al menos uno antes de generar el Word.');
  ...
  // Después de la tabla de actividades la plantilla trae, sueltas, las tablas
  // de los días de EJEMPLO de otro informe (FECHA: 16/07, 17/07... y la de
  // "AÑADIR NUEVA VENTANA") — se borran, si no quedaban en el Word final
  // mezcladas con las fechas reales. Y los párrafos vacíos que sobran justo
  // antes del salto de página que abre la sección siguiente también se
  // sacan: esos son los que empujaban una hoja en blanco entera.
  {
    let cursor = xml.indexOf('</w:tbl>', headerRowEndIdx + rowsXml.length + bookmarksHuerfanosParada.length) + '</w:tbl>'.length;
    for (;;) {
      const proxTbl = xml.indexOf('<w:tbl>', cursor);
      if (proxTbl === -1) break;
      if (/<w:t[ >]|<w:drawing|<w:br |<w:sectPr/.test(xml.slice(cursor, proxTbl))) break;
      let profundidad = 0, fin = -1;
      const reTbl = /<w:tbl>|<\/w:tbl>/g;
      reTbl.lastIndex = proxTbl;
      let mt;
      while ((mt = reTbl.exec(xml))) {
        if (mt[0] === '<w:tbl>') profundidad++;
        else if (--profundidad === 0) { fin = mt.index + '</w:tbl>'.length; break; }
      }
      if (fin === -1) break;
      const textoTabla = xml.slice(proxTbl, fin).replace(/<[^>]+>/g, '').trim();
      if (!/^(FECHA:|AÑADIR NUEVA VENTANA)/.test(textoTabla)) break;
      const bmDia = extraerBookmarkEndsHuerfanos(xml.slice(cursor, fin));
      xml = xml.slice(0, cursor) + bmDia + xml.slice(fin);
      cursor += bmDia.length;
    }
    const reParrafo = /<w:p[ >][\s\S]*?<\/w:p>/y;
    let pos = cursor, finVacios = cursor, hayVacios = false;
    for (;;) {
      reParrafo.lastIndex = pos;
      const mp = reParrafo.exec(xml);
      if (!mp) break;
      if (/<w:br w:type="page"/.test(mp[0])) {
        if (hayVacios) xml = xml.slice(0, cursor) + xml.slice(finVacios);
        break;
      }
      if (/<w:t[ >]|<w:drawing|<w:bookmark|<w:sectPr/.test(mp[0])) break;
      pos = finVacios = reParrafo.lastIndex;
      hayVacios = true;
    }
  }
  ...
  for (const { entry } of entradas) {
    ...
    const fechaTexto = fechaDDMMYYYY(entry.fecha);
    for (const [fotoIdx, foto] of entry.fotos.entries()) {
      const preparada = foto.url ? await prepararFotoParaWord(foto.url) : null;
      // Un espacio sin foto ni texto solo se ve en el editor.
      if (!preparada && !opciones.modoEditor && !(foto.descripcion || '').trim()) continue;
      let drawingXml = '';
...

  {
    ...
    // La plantilla trae este título (el de las fotos de la PARADA) copiado
    // del de preparativos: dice "...EN PERIODO DE PREPARATIVOS". Se corrige.
    const tituloFotosIdx = xml.indexOf('PERIODO DE PREPARATIVOS', fotosAnclaIdx);
    if (tituloFotosIdx !== -1 && tituloFotosIdx - fotosAnclaIdx < 400) {
      xml = xml.slice(0, tituloFotosIdx) + 'PERIODO DE PARADA' + xml.slice(tituloFotosIdx + 'PERIODO DE PREPARATIVOS'.length);
    }
    ...
    if (primerImagenIdx !== -1) {
      ...
      const fotosRowsXml = (fotosEmbebidas.length ? buildBloqueRegistroFotosWord(fotosEmbebidas, opciones.modoEditor) : '')
        + (opciones.modoEditor ? buildFilaMarcaEditorWord('⟦AÑADIR:fotos⟧') : '');
  ...
  // ---- Anexos: reemplaza DIAGRAMAS / PROTOCOLOS de ejemplo por los reales
  // (Diagramas, Planos, Protocolos — en ese orden). En el editor se dejan los
  // tres títulos siempre, con un botón para añadir; en el Word final solo
  // van los que tienen algo. Si algo falla, se sigue sin esto — nunca debe
  // tumbar el resto del informe. ----
  try {
    const reDiag = /<w:pStyle w:val="Ttulo2"\s*\/>(?:(?!<\/w:p>)[\s\S]){0,600}?<w:t>DIAGRAMAS<\/w:t>/;
    const rePro = /<w:pStyle w:val="Ttulo2"\s*\/>(?:(?!<\/w:p>)[\s\S]){0,600}?<w:t>PROTOCOLOS<\/w:t>/;
    const mDiag = reDiag.exec(xml);
    const mPro = rePro.exec(xml);
    if (mDiag && mPro) {
      const regionIni = xml.lastIndexOf('<w:p ', mDiag.index);
      const protTituloFin = xml.indexOf('</w:p>', mPro.index) + '</w:p>'.length;
      let regionFin = protTituloFin;
      const p2Ini = xml.indexOf('<w:p ', protTituloFin);
      if (p2Ini !== -1 && xml.slice(protTituloFin, p2Ini).trim() === '') {
        const p2Fin = xml.indexOf('</w:p>', p2Ini) + '</w:p>'.length;
        if (xml.slice(p2Ini, p2Fin).includes('<w:drawing')) regionFin = p2Fin;
      }

      const anexosTodos = (informe.anexos || []).map((a, i) => ({ ...a, _i: i }));
      const porCategoria = {};
      ANEXOS_CATEGORIAS.forEach((c) => { porCategoria[c.id] = []; });
      anexosTodos.forEach((a) => (porCategoria[a.categoria] || porCategoria.diagrama).push(a));

      let nuevoXml = '';
      let hayContenidoPrevio = false;
      for (const cat of ANEXOS_CATEGORIAS) {
        const lista = porCategoria[cat.id];
        if (!lista.length && !opciones.modoEditor) {
          // Sin anexos de este tipo: no va el título, pero se conserva su
          // marcador (el índice del documento apunta a él).
          if (cat.bookmark) nuevoXml += `<w:p><w:bookmarkStart w:id="${cat.bookmark.id}" w:name="${cat.bookmark.name}"/><w:bookmarkEnd w:id="${cat.bookmark.id}"/></w:p>`;
          continue;
        }
        ...
        for (const [k, anexo] of lista.entries()) {
          numero++;
          const texto = anexo.descripcion || (anexo.nombre || '').replace(/\.[^.]+$/, '');
          const marca = opciones.modoEditor ? `⟦X:anexo:${anexo._i}⟧` : '';
          const esImagen = (anexo.tipo || '').startsWith('image/') || /^data:image\//.test(anexo.url || '');
          const esPdf = !esImagen && ((anexo.tipo || '') === 'application/pdf' || /^data:application\/pdf/.test(anexo.url || '') || /\.pdf$/i.test(anexo.nombre || ''));
          // Un PDF se incrusta como imágenes: una hoja del Word por cada página del PDF.
          const pdfPaginas = esPdf ? await rasterizarPdfParaWord(anexo.url, Math.round(16 * INFORME_EMU_POR_CM), Math.round(19 * INFORME_EMU_POR_CM)) : null;
          if (pdfPaginas && pdfPaginas.paginas.length) {
            for (const [j, pg] of pdfPaginas.paginas.entries()) {
              contadorImagen++;
              const nombreArchivo = `anexopdf${Date.now()}_${contadorImagen}.jpeg`;
              const rId = `rId${nextRid++}`;
          ...
          const preparada = esImagen ? await prepararImagenAnexoParaWord(anexo.url, Math.round(16 * INFORME_EMU_POR_CM), Math.round(19 * INFORME_EMU_POR_CM)) : null;
          if (preparada) {
            contadorImagen++;
            const nombreArchivo = `anexo${Date.now()}_${contadorImagen}.jpeg`;
            const rId = `rId${nextRid++}`;
            zip.file(`word/media/${nombreArchivo}`, preparada.bytes);
            nuevasRelsXml += `<Relationship Id="${rId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/${nombreArchivo}"/>`;
            ...
            nuevoXml += buildLeyendaAnexoWord(cat.etiqueta, numero, aviso, marca);
          }
        }
        if (opciones.modoEditor) nuevoXml += buildMarcaEditorWord(`⟦AÑADIR:${cat.id}⟧`);
      }
      // Lo que traía la plantilla al final de la sección era un salto de
      // página (para abrir la sección que sigue) — se conserva, dentro del
      // último párrafo, para no dejar ninguna hoja en blanco de por medio.
      const saltoFinal = '<w:r><w:br w:type="page"/></w:r>';
      const ultimoCierre = nuevoXml.lastIndexOf('</w:p>');
      nuevoXml = ultimoCierre === -1
        ? `<w:p>${saltoFinal}</w:p>`
        : nuevoXml.slice(0, ultimoCierre) + saltoFinal + nuevoXml.slice(ultimoCierre);
      xml = xml.slice(0, regionIni) + nuevoXml + xml.slice(regionFin);
    }
  } catch (e) {
    console.error('No se pudieron agregar los anexos en el Word:', e);
  }
  ...
  try {
    ...
    if (cronogramaAnclaIdx !== -1) {
      ...
      if (drawingStartIdx !== -1 && drawingStartIdx - cronogramaAnclaIdx < 6000) {
        ...
        const htmlCurva = `<div style="display:flex; align-items:flex-start; gap:20px;">
        ...
        const imgCurva = await htmlAPngCacheado('curva|' + htmlCurva, 'position:fixed; left:-10000px; top:0; width:900px; background:#ffffff; font-family:Arial,Helvetica,sans-serif; color:#1A1A2E; padding:16px 22px;', htmlCurva);
        if (imgCurva) {
          const bytesCurva = imgCurva.bytes;
          const canvasCurva = { width: imgCurva.w, height: imgCurva.h };
  ...
  // ---- Con un título muy largo la hoja del cronograma se llena completa y su
  // salto de página (un párrafo de una línea) caía en una hoja en blanco. Ese
  // salto se cambia por uno de 1 pt de alto (igual que en la hoja de cumplimiento). ----
  try {
    const cronoIdx = xml.indexOf('En base a la Carta Gantt de');
    if (cronoIdx !== -1) {
      const reSalto = /<w:p(?: [^>]*)?>(?:<w:pPr>(?:(?!<\/w:pPr>)[\s\S])*<\/w:pPr>)?<w:r(?: [^>]*)?>(?:<w:rPr>(?:(?!<\/w:rPr>)[\s\S])*<\/w:rPr>)?<w:br w:type="page"\/><\/w:r><\/w:p>/g;
      reSalto.lastIndex = cronoIdx;
      const m = reSalto.exec(xml);
      const limite = xml.indexOf('DESCRIPCIÓN DEL TRABAJO', cronoIdx);
      if (m && (limite === -1 || m.index < limite)) {
        xml = xml.slice(0, m.index) + SALTO_PAGINA_MINIMO_XML + xml.slice(m.index + m[0].length);
      }
    }
  } catch (e) {
    console.error('No se pudo ajustar el salto de página del cronograma:', e);
  }

  // ---- Listas finales: componentes (los mismos que se cargan en cada
  // actividad de la app), herramientas y consumibles (texto del informe). ----
  try {
    const modoEd = !!opciones.modoEditor;
    const otsSetLista = new Set(ots.map((o) => String(o.otNum)));
    const comps = state.componentes
      .filter((c) => otsSetLista.has(String(c.otNum)) && (modoEd || (c.descripcion || '').trim()))
      .sort((a, b) => String(a.area || '').localeCompare(String(b.area || '')) || (Number(a.otNum) - Number(b.otNum)) || ((a.createdAt || 0) - (b.createdAt || 0)));
    const filasComp = comps.map((c) => {
      const base = (c.descripcion || '').trim();
      const sap = (c.codigoSAP || '').trim();
      const texto = (base || (modoEd ? '(toca aquí para escribir el componente)' : '')) + (sap ? ` (SAP ${sap})` : '');
      return { desc: texto + (modoEd ? ` ⟦EDIT:comp:${c.id}⟧ ⟦X:comp:${c.id}⟧` : ''), cant: String(c.cantidad == null || c.cantidad === '' ? 1 : c.cantidad) };
    });
    xml = reemplazarFilasListaWord(xml, '_Toc237061364', filasComp, true, modoEd ? '⟦AÑADIR:comp⟧' : '');
    xml = reemplazarFilasListaWord(xml, '_Toc237061365', parsearHerramientasTexto(informe.herramientasTexto), true, modoEd ? '⟦EDIT:herr⟧' : '');
    const filasCons = String(informe.consumiblesTexto || '').split('\n').map((l) => l.trim()).filter(Boolean).map((l) => ({ desc: l }));
    xml = reemplazarFilasListaWord(xml, '_Toc237061366', filasCons, false, modoEd ? '⟦EDIT:cons⟧' : '');
  } catch (e) {
    console.error('No se pudieron armar las listas de componentes/herramientas/consumibles:', e);
  }

  // ---- Espacio entre la tabla de administradores/supervisores y el título
  // "3. PROGRAMA DEL SERVICIO": esa tabla es flotante y el título quedaba
  // pegado a su borde inferior. Se le da aire abajo a la tabla y arriba al
  // título. ----
  try {
    const bmPrograma = xml.indexOf('w:name="_Toc237061354"');
    if (bmPrograma !== -1) {
      const pTitulo = xml.lastIndexOf('<w:p ', bmPrograma);
      const tblpIdx = xml.lastIndexOf('<w:tblpPr ', pTitulo);
      if (tblpIdx !== -1 && tblpIdx > xml.lastIndexOf('<w:tbl>', pTitulo) && !/^<w:tblpPr [^>]*bottomFromText/.test(xml.slice(tblpIdx, tblpIdx + 300))) {
        xml = xml.slice(0, tblpIdx) + '<w:tblpPr w:bottomFromText="454" ' + xml.slice(tblpIdx + '<w:tblpPr '.length);
      }
      const pTituloNuevo = xml.lastIndexOf('<w:p ', xml.indexOf('w:name="_Toc237061354"'));
      const finPPr = xml.indexOf('</w:pPr>', pTituloNuevo);
      xml = xml.slice(0, pTituloNuevo) + xml.slice(pTituloNuevo, finPPr).replace('w:before="240"', 'w:before="600"') + xml.slice(finPPr);
    }
  } catch (e) {
    console.error('No se pudo separar el título "Programa del servicio":', e);
  }
  ...
  try {
    ...
    if (introIdx !== -1) {
      ...
      if (headerRowEndConclusiones !== -1 && tblEndConclusiones !== -1) {
        ...
        xml = xml.slice(0, headerRowEndConclusiones) + filasConclusiones + bookmarksHuerfanosConcl
          + xml.slice(tblEndConclusiones, tblEndConclusiones + '</w:tbl>'.length)
          + (opciones.modoEditor ? buildMarcaEditorWord('⟦EDIT:conclall⟧') : '')
          + xml.slice(tblEndConclusiones + '</w:tbl>'.length);
      }
    }
  } catch (e) {
  ...
  try {
    ...
    // En el editor también se ven las recién creadas (aún sin texto).
    const otsConRecomendacion = ots.filter((ot) => (opciones.modoEditor
      ? Object.prototype.hasOwnProperty.call(recomendaciones, ot.otNum)
      : (recomendaciones[ot.otNum] || '').trim()));
    ...
    if (introIdx !== -1) {
      ...
      if (headerRowEndRecomendaciones !== -1 && tblEndRecomendaciones !== -1) {
        const filasRecomendaciones = otsConRecomendacion.map((ot) => buildFilaRecomendacionWord(ot, (recomendaciones[ot.otNum] || '').trim(), opciones.modoEditor)).join('');
        const bookmarksHuerfanosRecom = extraerBookmarkEndsHuerfanos(xml.slice(headerRowEndRecomendaciones, tblEndRecomendaciones));
        // El encabezado de esta tabla venía copiado de la de conclusiones:
        // dice "CONCLUSIONES" y debe decir "RECOMENDACIONES".
        const encabezadoRecom = xml.slice(tblStart, headerRowEndRecomendaciones).replace('>CONCLUSIONES<', '>RECOMENDACIONES<');
        xml = xml.slice(0, tblStart) + encabezadoRecom + filasRecomendaciones + bookmarksHuerfanosRecom
          + xml.slice(tblEndRecomendaciones, tblEndRecomendaciones + '</w:tbl>'.length)
          + (opciones.modoEditor ? buildMarcaEditorWord('⟦AÑADIR:reco⟧') : '')
          + xml.slice(tblEndRecomendaciones + '</w:tbl>'.length);
      }
    }
  } catch (e) {
  ...
  // Word (a diferencia de la vista previa) NO abre un documento donde una
  // celda de tabla no termina en un párrafo — lo declara "corrupto". Es un
  // error fácil de cometer al meter tablas dentro de celdas, así que se
  // revisa acá siempre, antes de entregar el archivo.
  const W_NS = 'http://schemas.openxmlformats.org/wordprocessingml/2006/main';
  const celdasMalas = Array.from(parseCheck.getElementsByTagNameNS(W_NS, 'tc')).filter((tc) => {
    const hijos = Array.from(tc.childNodes).filter((n) => n.nodeType === 1);
    return !hijos.length || hijos[hijos.length - 1].localName !== 'p';
  });
  if (celdasMalas.length) {
    throw new Error(`El Word generado tiene ${celdasMalas.length} celda(s) de tabla mal formada(s) (no terminan en párrafo) — no se generó el archivo para no entregar un Word que no abre.`);
  }

  zip.file('word/document.xml', xml);
  relsXml = relsXml.replace('</Relationships>', nuevasRelsXml + '</Relationships>');
  // Imágenes / objetos incrustados de la plantilla que ya no se usan (los
  // ejemplos que se reemplazaron, como el protocolo de muestra de 36 MB) se
  // sacan del archivo — si no, el Word pesa 10 MB de más sin mostrar nada.
  const otrosRels = zip.file(/^word\/_rels\/(?!document\.xml\.rels$).*\.rels$/).map((f) => f.asText()).join('\n');
  relsXml = relsXml.replace(/<Relationship\s[^>]*\/>/g, (rel) => {
    const id = (/\sId="([^"]+)"/.exec(rel) || [])[1];
    const tipo = (/\sType="([^"]+)"/.exec(rel) || [])[1] || '';
    const destino = (/\sTarget="([^"]+)"/.exec(rel) || [])[1] || '';
    if (!id || !/\/(image|package|oleObject)$/.test(tipo) || !/^(media|embeddings)\//.test(destino)) return rel;
    if (xml.includes(`"${id}"`)) return rel;
    if (!otrosRels.includes(`Target="${destino}"`) && !relsXml.replace(rel, '').includes(`Target="${destino}"`)) zip.remove(`word/${destino}`);
    return '';
  });
  {
...
// Descarga un blob (Word o PDF ya armado) — en un navegador normal usa el
// truco de siempre (link con download + clic); dentro del APK (Capacitor)
...
async function abrirVistaPreviaWord(informe) {
  ...
  try {
    ...
    quitarHojasVaciasVisor(contenedor);
    centrarTablasAnchasVisor(contenedor);
    await ajustarMarcaAguaEditor(contenedor);
...
// ============================================================
// "Editar como documento" — pantalla completa con el documento real
// (mismo motor que la vista previa) donde los únicos puntos tocables son
// los que se pueden llenar: título, foto de portada, la tabla de actividades,
// y los botones de añadir/quitar (días, filas de fotos, anexos). El resto del
// documento se ve exactamente igual pero no se toca.
//
// Cada cambio se guarda por el mismo camino de siempre (bitácora, campos del
// informe) — así lo que se edita acá y lo que se carga en el otro formulario
// es SIEMPRE lo mismo — y el documento se vuelve a generar, sin perder el
// punto donde se estaba leyendo.
// ============================================================

let editorRefrescando = false;
let editorPendiente = false;

function otsDelInforme(informe) {
  return (informe.otNums || []).map((n) => allOts().find((o) => String(o.otNum) === String(n))).filter(Boolean);
}
function etiquetaOtEditor(ot) { return ot.manual ? ot.descripcion : `OT ${ot.otNum} — ${tituloSinEquipo(ot.descripcion)}`; }
function hoyISOEditor() { return new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 10); }
...
async function abrirVistaInteractiva(informe) {
  ...
  await renderizarDocumentoEditor(true);
}

async function refrescarVistaInteractiva() {
  if (!document.getElementById('vistaInteractivaBackdrop').classList.contains('open')) return;
  await renderizarDocumentoEditor(false);
}

// Se dibuja en un contenedor aparte (fuera de pantalla) y recién cuando está
// listo se cambia por el de la vista — así no parpadea ni se pierde el punto
// del documento donde se estaba (scroll).
async function renderizarDocumentoEditor(primeraVez) {
  if (editorRefrescando) { editorPendiente = true; return; }
  editorRefrescando = true;
  const cont = document.getElementById('vistaInteractivaContenedor');
  const loading = document.getElementById('vistaInteractivaLoading');
  const estado = document.getElementById('editorEstado');
  const destacar = editorDestacar;
  editorDestacar = null;
  const barra = document.querySelector('.editor-barra');
  if (primeraVez) { cont.innerHTML = ''; loading.style.display = 'block'; }
  else {
    // El documento actual se queda a la vista (atenuado, sin poder tocarlo) y
    // una barra fina avisa que se está actualizando — nada de saltos ni pantalla en blanco.
    cont.classList.add('editor-actualizando');
    if (barra) barra.classList.add('editor-cargando');
  }
  const scroll = cont.scrollTop;
  const staging = document.createElement('div');
  staging.style.cssText = `position:absolute; left:-100000px; top:0; width:${Math.max(cont.clientWidth, 900)}px;`;
  document.body.appendChild(staging);
  try {
    const informe = state.informeActivo;
    const { blob } = await generateInformeWordBlob(informe, { modoEditor: true });
    await window.docx.renderAsync(blob, staging, staging, { className: 'docx-preview', inWrapper: true });
    // Todos los ajustes se hacen con el documento nuevo todavía fuera de
    // pantalla: así al cambiarlo ya aparece terminado, sin el parpadeo de la
    // marca de agua o las tablas corriéndose de lugar.
    quitarHojasVaciasVisor(staging);
    centrarTablasAnchasVisor(staging);
    ...
    if (!primeraVez) {
      cont.classList.remove('editor-actualizando');
      cont.classList.add('editor-entra');
      setTimeout(() => cont.classList.remove('editor-entra'), 400);
    }
    if (destacar) resaltarBloqueNuevoEditor(cont, destacar);
  } catch (e) {
    console.error(e);
    showToast(e.message || 'No se pudo armar el documento');
  } finally {
    staging.remove();
    cont.classList.remove('editor-actualizando');
    if (barra) barra.classList.remove('editor-cargando');
    loading.style.display = 'none';
    estado.textContent = '';
    editorRefrescando = false;
    if (editorPendiente) { editorPendiente = false; renderizarDocumentoEditor(false); }
  }
}

// Lo último que se añadió (día o fila de fotos) se lleva a la vista con un
// desplazamiento suave y un destello, para que se note dónde quedó.
let editorDestacar = null;
function resaltarBloqueNuevoEditor(cont, que) {
  let el = null;
  if (que === 'dia') {
    const pasos = Array.from(cont.querySelectorAll('.doc-editable')).filter((p) => /toca aquí para escribir el paso/.test(p.textContent));
    el = pasos[pasos.length - 1];
    if (el) el = el.closest('td') || el;
  } else if (que && que.comp) {
    const x = Array.from(cont.querySelectorAll('.doc-btn-quitar')).find((b) => b.dataset.refs === que.comp);
    el = x ? x.closest('tr') : null;
  } else if (que && que.reco) {
    const x = Array.from(cont.querySelectorAll('.doc-btn-quitar')).find((b) => b.dataset.refs === que.reco);
    el = x ? x.closest('tr') : null;
  } else if (que && que.refs) {
    const x = Array.from(cont.querySelectorAll('.doc-btn-quitar')).find((b) => b.dataset.refs && b.dataset.refs.startsWith(que.refs));
    el = x ? x.closest('table') : null;
  }
  if (!el) return;
  el.scrollIntoView({ behavior: 'smooth', block: 'center' });
  el.classList.add('doc-nuevo');
  setTimeout(() => el.classList.remove('doc-nuevo'), 1600);
}

// Una tabla centrada que es más ancha que el área de texto: Word la deja
// centrada (sobra lo mismo a cada lado), docx-preview la pega a la izquierda y
// se sale toda hacia la derecha — el "desfase" de las tablas. Se corrige
// corriéndola la mitad de lo que sobra.
function centrarTablasAnchasVisor(cont) {
  cont.querySelectorAll('section.docx-preview > article, section.docx-preview > header, section.docx-preview > footer').forEach((art) => {
    const anchoArt = art.clientWidth;
    Array.from(art.children).forEach((el) => {
      if (el.tagName !== 'TABLE' || el.style.marginLeft !== 'auto') return;
      const sobra = el.offsetWidth - anchoArt;
      if (sobra > 1) el.style.marginLeft = `${-sobra / 2}px`;
    });
  });
}

// docx-preview arma una hoja vacía cuando un salto de página cae justo
// después de otro (Word los junta en uno solo). Como esas hojas no existen en
// el Word real, se sacan del visor.
function quitarHojasVaciasVisor(cont) {
  Array.from(cont.querySelectorAll('section.docx-preview')).forEach((sec, i) => {
    if (i === 0) return;
    const art = sec.querySelector('article');
    if (!art) return;
    if (!art.textContent.trim() && !art.querySelector('img, svg, table')) sec.remove();
  });
}

// docx-preview no acomoda bien la marca de agua del encabezado: en Word esa
// imagen está anclada a la HOJA (borde izquierdo, y a tantos cm desde arriba),
// pero docx-preview la mide desde donde termina el encabezado y la deja corrida
// hacia abajo. Acá se vuelve a anclar a la hoja con los mismos datos del Word
// y, como en esta vista una "hoja" puede ser más larga que una A4 (docx-preview
// solo corta en los saltos explícitos), se repite una por cada alto de A4.
async function ajustarMarcaAguaEditor(cont) {
  const altoA4 = 841.9 * 96 / 72;
  // docx-preview asigna el src de las imágenes un instante después de terminar
  // de dibujar; se espera a que estén todas listas antes de copiar la marca.
  for (let i = 0; i < 40 && Array.from(cont.querySelectorAll('header img')).some((im) => !im.getAttribute('src')); i++) {
    await new Promise((r) => setTimeout(r, 50));
  }
  cont.querySelectorAll('section.docx-preview').forEach((sec) => {
    const hd = sec.querySelector('header');
    if (!hd) return;
    const marca = Array.from(hd.querySelectorAll('img')).find((im) => parseFloat(im.style.width) > 300);
    if (!marca) return;
    const ptAPx = (v) => (parseFloat(v) || 0) * 96 / 72;
    const contenedorMarca = marca.parentElement;
    const w = ptAPx(marca.style.width);
    const h = ptAPx(marca.style.height);
    const izq = contenedorMarca ? ptAPx(contenedorMarca.style.left) : 0;
    const desdeArriba = contenedorMarca && contenedorMarca.style.top ? ptAPx(contenedorMarca.style.top) : altoA4 - h;
    if (contenedorMarca) contenedorMarca.style.display = 'none';
    sec.style.position = 'relative';
    sec.style.isolation = 'isolate';
    sec.querySelectorAll('.marca-agua-ed').forEach((n) => n.remove());
    const alto = sec.getBoundingClientRect().height;
    const nPag = Math.max(1, Math.round(alto / altoA4));
    for (let k = 0; k < nPag; k++) {
      const clon = document.createElement('img');
      clon.className = 'marca-agua-ed';
      clon.src = marca.src;
      clon.style.cssText = `position:absolute; z-index:-1; pointer-events:none; width:${w}px; height:${h}px; left:${izq}px; top:${k * altoA4 + desdeArriba}px;`;
      sec.appendChild(clon);
    }
  });
}

// En pantallas angostas (celular) la hoja completa no cabe: se achica para
// que se vea entera de lado a lado, como en Word.
function ajustarZoomEditor(cont) {
  const envoltorio = cont.querySelector('.docx-preview-wrapper') || cont.firstElementChild;
  const pagina = cont.querySelector('section.docx-preview');
  if (!envoltorio || !pagina) return;
  envoltorio.style.zoom = '';
  const disponible = cont.clientWidth - 16;
  const ancho = pagina.offsetWidth;
  if (ancho > disponible) envoltorio.style.zoom = String(disponible / ancho);
}

// Busca, dentro del render real, los nodos de texto que dicen exactamente
// `texto` y les agrega un borde punteado + evento de clic. El mismo dato
// suele repetirse varias veces en el documento (encabezado de cada página,
// portada, índice) — se marcan TODAS las repeticiones a la vez.
function resaltarTextoEditable(contenedor, texto, onClick, tituloAyuda) {
  if (!texto) return [];
  const walker = document.createTreeWalker(contenedor, NodeFilter.SHOW_TEXT);
  const nodos = [];
  while (walker.nextNode()) {
    if (walker.currentNode.nodeValue.includes(texto)) nodos.push(walker.currentNode);
  }
  nodos.forEach((nodo) => {
    const el = nodo.parentElement;
    el.classList.add('doc-editable');
    el.title = tituloAyuda || 'Toca para editar';
    el.addEventListener('click', onClick);
  });
  return nodos;
}

function activarPuntosEditablesInforme(informe, contenedor) {
  const tituloActual = informe.tituloGeneral ? informe.tituloGeneral.toUpperCase() : null;
  if (tituloActual) {
    resaltarTextoEditable(contenedor, tituloActual,
      () => abrirEdicionTextoInforme(informe, 'tituloGeneral', 'Título del trabajo', informe.tituloGeneral),
      'Toca para cambiar el título');
  } else {
    resaltarTextoEditable(contenedor, 'DEL SERVICIO DE:',
      () => abrirEdicionTextoInforme(informe, 'tituloGeneral', 'Título del trabajo', ''),
      'Toca para escribir el título');
  }

  // La foto de portada es la primera imagen GRANDE que viene después del
  // texto "DEL SERVICIO DE:" — la primera <img> del documento es otra cosa
  // (imagen decorativa de fondo de la plantilla), no sirve.
  const walkerAncla = document.createTreeWalker(contenedor, NodeFilter.SHOW_TEXT);
  let ancla = null;
  while (walkerAncla.nextNode()) {
    if (walkerAncla.currentNode.nodeValue.includes(PORTADA_FOTO_ANCLA)) { ancla = walkerAncla.currentNode; break; }
  }
  const despuesDelAncla = Array.from(contenedor.querySelectorAll('img')).filter(
    (im) => ancla && (ancla.compareDocumentPosition(im) & Node.DOCUMENT_POSITION_FOLLOWING));
  const foto = despuesDelAncla.find((im) => im.getBoundingClientRect().width > 150) || despuesDelAncla[0];
  if (foto) {
    foto.classList.add('doc-editable-img');
    foto.title = 'Toca para cambiar la foto';
    foto.addEventListener('click', () => abrirSelectorFotoInforme(informe, 'portadaFotoUrl'));
  }

  // Tabla TAG / OT / DESCRIPCIÓN / % EJECUCIÓN: tocarla abre la lista con
  // check de las actividades que van en este informe.
  const tablaCumplimiento = Array.from(contenedor.querySelectorAll('table')).find((t) => {
    const txt = t.textContent;
    return txt.includes('TAG') && txt.includes('DESCRIPCIÓN') && txt.includes('EJECUCIÓN');
  });
  if (tablaCumplimiento) {
    tablaCumplimiento.classList.add('doc-editable-tabla');
    tablaCumplimiento.title = 'Toca para elegir las actividades de este informe';
    tablaCumplimiento.addEventListener('click', dialogoActividadesInforme);
  }
}

// ---- Botones dentro del documento: las marcas ⟦...⟧ que deja la generación
// (solo en el editor) se reemplazan por botones de verdad. ----
const CONTROLES_ANADIR = {
  dia: { texto: '＋ Añadir día', fn: () => anadirDiaDirecto() },
  fotos: { texto: '＋ Añadir fila de fotos', fn: () => anadirFilaFotosDirecto('parada') },
  fotosprep: { texto: '＋ Añadir fila de fotos', fn: () => anadirFilaFotosDirecto('prep') },
  distrib: { texto: '＋ Añadir imagen', fn: () => dialogoAnadirDistribucion() },
  reco: { texto: '＋ Añadir recomendación', fn: () => anadirRecomendacionDirecto() },
  comp: { texto: '＋ Añadir componente', fn: () => anadirComponenteDirecto() },
  diagrama: { texto: '＋ Añadir diagrama', fn: () => dialogoAnadirAnexo('diagrama') },
  plano: { texto: '＋ Añadir plano', fn: () => dialogoAnadirAnexo('plano') },
  protocolo: { texto: '＋ Añadir protocolo', fn: () => dialogoAnadirAnexo('protocolo') },
};

function activarMarcasEditor(informe, cont) {
  const walker = document.createTreeWalker(cont, NodeFilter.SHOW_TEXT);
  const nodos = [];
  while (walker.nextNode()) { if (walker.currentNode.nodeValue.includes('⟦')) nodos.push(walker.currentNode); }
  nodos.forEach((nodoInicial) => {
    let nodo = nodoInicial;
    let m;
    while ((m = /⟦([^⟧]+)⟧/.exec(nodo.nodeValue))) {
      const partes = m[1].split(':');
      const tipo = partes[0], sub = partes[1], resto = partes.slice(2).join(':');
      const antes = nodo.nodeValue.slice(0, m.index);
      const despues = nodo.nodeValue.slice(m.index + m[0].length);

      // Textos que se editan tocándolos: la marca desaparece y el párrafo
      // donde estaba queda como punto editable (borde punteado).
      const hotspot = HOTSPOTS_EDITOR[sub];
      if (tipo === 'EDIT' && hotspot) {
        nodo.nodeValue = antes + despues;
        const el = nodo.parentElement.closest('p') || nodo.parentElement;
        el.classList.add('doc-editable');
        el.title = 'Toca para editar';
        el.addEventListener('click', (e) => { e.stopPropagation(); hotspot(resto); });
        continue;
      }
      ...
      btn.type = 'button';
      if (tipo === 'X') {
        btn.className = 'doc-btn-quitar';
        btn.textContent = '✕';
        btn.title = 'Quitar';
        btn.dataset.refs = resto;
        btn.addEventListener('click', (e) => { e.stopPropagation(); quitarDesdeEditor(sub, resto); });
      } else if (tipo === 'FOTO' && sub === 'del') {
        btn.className = 'doc-btn-quitar';
        btn.textContent = '✕';
        btn.title = 'Quitar esta foto';
        btn.addEventListener('click', (e) => { e.stopPropagation(); quitarFotoDeEspacio(resto); });
      } else if (tipo === 'FOTO') {
        btn.className = 'doc-btn-anadir';
        btn.textContent = '＋ Añadir foto';
        btn.addEventListener('click', (e) => { e.stopPropagation(); if (sub === 'add') fotoEnEspacio(resto); else fotoNuevaEnGrupo(resto); });
      } else if (tipo === 'EDIT' && (sub === 'herr' || sub === 'cons')) {
        btn.className = 'doc-btn-anadir';
        btn.textContent = sub === 'herr' ? '✎ Editar herramientas' : '✎ Editar consumibles';
        btn.addEventListener('click', () => dialogoEditarListaTexto(sub));
      } else if (tipo === 'EDIT' && sub === 'conclall') {
        btn.className = 'doc-btn-anadir';
        ...
        btn.addEventListener('click', dialogoEditarPreparativos);
      } else if (sub === 'paso') {
        btn.className = 'doc-btn-anadir doc-btn-chico';
        btn.textContent = '＋ Añadir paso';
        btn.addEventListener('click', () => anadirPasoDirecto(resto));
      } else {
        const ctl = CONTROLES_ANADIR[sub];
        btn.className = 'doc-btn-anadir';
        btn.textContent = ctl ? ctl.texto : '＋ Añadir';
        btn.addEventListener('click', () => { if (ctl) ctl.fn(); });
      }
      nodo.nodeValue = antes;
      nodo.parentNode.insertBefore(btn, nodo.nextSibling);
      if (despues.trim()) {
        const cola = document.createTextNode(despues);
        nodo.parentNode.insertBefore(cola, btn.nextSibling);
        nodo = cola;
      } else break;
    }
  });
...
// Los textos del documento que se editan tocándolos (marca ⟦EDIT:<tipo>:...⟧).
const HOTSPOTS_EDITOR = {
  desc: (ref) => dialogoEditarDescripcion(ref),
...
// ---- Diálogo genérico del editor ----
function abrirDialogoEditor({ titulo, campos, textoGuardar = 'Guardar', onGuardar, accionExtra }) {
  const previo = document.getElementById('dialogoEditor');
  if (previo) previo.remove();
  const bd = document.createElement('div');
  bd.id = 'dialogoEditor';
  bd.className = 'editor-dialogo-backdrop';
  const cuerpo = campos.map((c) => {
    const id = 'de_' + c.id;
    const lab = `<label for="${id}">${escBit(c.etiqueta)}</label>`;
    if (c.tipo === 'textarea') return `${lab}<textarea id="${id}" rows="${c.filas || 4}" placeholder="${escBit(c.placeholder || '')}">${escBit(c.valor || '')}</textarea>`;
    if (c.tipo === 'select') return `${lab}<select id="${id}">${c.opciones.map((o) => `<option value="${escBit(String(o.valor))}"${String(o.valor) === String(c.valor) ? ' selected' : ''}>${escBit(o.texto)}</option>`).join('')}</select>`;
    if (c.tipo === 'file') return `${lab}<input type="file" id="${id}" accept="${c.accept || 'image/*'}">`;
    if (c.tipo === 'checklist') {
      return `${lab}<div id="${id}" class="editor-checklist">${c.grupos.map((g) => `<div class="editor-check-grupo">${escBit(g.titulo)}</div>${g.items.map((it) => `<label class="editor-check"><input type="checkbox" value="${escBit(String(it.valor))}"${it.marcado ? ' checked' : ''}><span>${escBit(it.texto)}</span></label>`).join('')}`).join('')}</div>`;
    }
    return `${lab}<input type="${c.tipo || 'text'}" id="${id}" value="${escBit(String(c.valor == null ? '' : c.valor)).replace(/"/g, '&quot;')}">`;
  }).join('');
  bd.innerHTML = `<div class="editor-dialogo"><h3>${escBit(titulo)}</h3><div class="editor-dialogo-cuerpo">${cuerpo}</div><p class="editor-dialogo-error"></p>${accionExtra ? `<button type="button" class="editor-dialogo-peligro" data-x>${escBit(accionExtra.texto)}</button>` : ''}<div class="popup-edicion-acciones"><button type="button" class="btn ghost" data-c>Cancelar</button><button type="button" class="btn primary" data-g>${escBit(textoGuardar)}</button></div></div>`;
  document.body.appendChild(bd);
  const cerrar = () => bd.remove();
  const errEl = bd.querySelector('.editor-dialogo-error');
  bd.querySelector('[data-c]').addEventListener('click', cerrar);
  bd.addEventListener('click', (e) => { if (e.target === bd) cerrar(); });
  const btnX = bd.querySelector('[data-x]');
  if (btnX) {
    btnX.addEventListener('click', async () => {
      try { await accionExtra.fn(); cerrar(); } catch (e) { errEl.textContent = 'No se pudo: ' + ((e && e.message) || 'error desconocido'); }
    });
  }
  const btnG = bd.querySelector('[data-g]');
  btnG.addEventListener('click', async () => {
    const valores = {};
    campos.forEach((c) => {
      const el = bd.querySelector('#de_' + c.id);
      if (c.tipo === 'file') valores[c.id] = el.files[0];
      else if (c.tipo === 'checklist') valores[c.id] = Array.from(el.querySelectorAll('input:checked')).map((i) => i.value);
      else valores[c.id] = el.value;
    });
    errEl.textContent = '';
    btnG.disabled = true; const txt = btnG.textContent; btnG.textContent = 'Guardando…';
    try {
      const err = await onGuardar(valores);
      if (typeof err === 'string' && err) { errEl.textContent = err; btnG.disabled = false; btnG.textContent = txt; return; }
      cerrar();
    } catch (e) {
      console.error(e);
      errEl.textContent = 'No se pudo guardar: ' + ((e && e.message) || 'error desconocido');
      btnG.disabled = false; btnG.textContent = txt;
    }
  });
...
function abrirEdicionTextoInforme(informe, campo, etiqueta, valorActual) {
  abrirDialogoEditor({
    titulo: etiqueta,
    campos: [{ id: 'valor', tipo: 'text', etiqueta, valor: valorActual || '' }],
    onGuardar: async (v) => {
      const valor = (v.valor || '').trim();
      if (!valor) return 'Escribe un valor antes de guardar';
      await guardarCampoInformeActivo(campo, valor);
      // El nombre del informe (el que se ve en la lista y en el formulario
      // clásico) lleva el título: se mantienen iguales.
      if (campo === 'tituloGeneral' && informe.numero) await guardarCampoInformeActivo('nombre', `IT-MCEN-${informe.numero}-SUL — ${valor}`);
...
// (informe.otNums) que usa el formulario de siempre, así no se desincroniza.
function dialogoActividadesInforme() {
  const informe = state.informeActivo;
  const marcadas = new Set((informe.otNums || []).map(String));
  const areas = [...new Set(allOts().map((o) => o.area))];
  abrirDialogoEditor({
    titulo: 'Actividades de este informe',
    campos: [{
      id: 'ots', tipo: 'checklist', etiqueta: 'Marca las actividades que van en este informe',
      grupos: areas.map((a) => ({
        titulo: a,
        items: allOts().filter((o) => o.area === a).map((o) => ({ valor: o.otNum, texto: etiquetaOtEditor(o), marcado: marcadas.has(String(o.otNum)) })),
      })),
    }],
    onGuardar: async (v) => {
      if (!v.ots.length) return 'Elige al menos una actividad';
      await guardarCampoInformeActivo('otNums', v.ots.map(String));
      await refrescarVistaInteractiva();
    },
  });
}

// El índice del turno (para ordenar y para la Curva S) sale de la fecha y el
// turno elegidos; si esa fecha no está en la parada, se usa el turno más
// cercano hacia adelante.
function turnoIdxDeFechaTurno(fechaISO, turnoTipo) {
  const tipo = turnoTipo === 'Día' ? 'A' : 'B';
  let idx = SEED_DATA.turnos.findIndex((iso, i) => iso.slice(0, 10) === fechaISO && turnoTipoDe(i) === tipo);
  if (idx !== -1) return idx;
  idx = SEED_DATA.turnos.findIndex((iso) => iso.slice(0, 10) >= fechaISO);
  return idx === -1 ? SEED_DATA.turnos.length - 1 : idx;
}

function camposActividadFechaTurno(ots) {
  return [
    { id: 'ot', tipo: 'select', etiqueta: 'Actividad', opciones: ots.map((o) => ({ valor: o.otNum, texto: etiquetaOtEditor(o) })) },
    { id: 'fecha', tipo: 'date', etiqueta: 'Fecha', valor: hoyISOEditor() },
    { id: 'turno', tipo: 'select', etiqueta: 'Turno', opciones: [{ valor: 'Día', texto: 'Día' }, { valor: 'Noche', texto: 'Noche' }] },
  ];
}

function datosBaseEntradaBitacora(ot, fecha, turno) {
  return {
    otNum: ot.otNum, otDescripcion: ot.descripcion, area: ot.area,
    titulo: ot.manual ? ot.descripcion : `OT ${ot.otNum} — ${ot.descripcion}`,
    fecha, turnoTipo: turno, turnoIdx: turnoIdxDeFechaTurno(fecha, turno), createdAt: Date.now(),
  };
}

// ================= Edición directa dentro del documento =================
// Añadir un día o una fila de fotos NO abre un formulario: crea de inmediato
// el bloque (con valores por defecto) y aparece en el documento con cada campo
...
function entradaBitacoraPorId(id) { return state.bitacora.find((b) => b.id === id); }
function fechaISOMasDias(iso, dias) {
  const d = new Date(iso + 'T12:00:00');
  d.setDate(d.getDate() + dias);
  return d.toISOString().slice(0, 10);
}
function avisarErrorEditor(e, prefijo) {
  console.error(e);
  showToast(`${prefijo}: ${(e && e.message) || 'error desconocido'}`);
}

// Guarda los pasos de una entrada; si se queda sin pasos ni fotos, la entrada
// ya no tiene nada que mostrar y se borra.
async function guardarPasosEntrada(entrada, bullets) {
  if (!bullets.length && !(entrada.fotos || []).length) await bitacoraCollection().doc(entrada.id).delete();
...
async function anadirPasoDirecto(id) {
  ...
  try { await bitacoraCollection().doc(id).update({ bullets }); } catch (err) { avisarErrorEditor(err, 'No se pudo añadir el paso'); }
  await refrescarVistaInteractiva();
  dialogoEditarPaso(id, bullets.length - 1);
}
...
function dialogoEditarActividadDia(id) {
  const e = entradaBitacoraPorId(id);
  const ots = otsDelInforme(state.informeActivo);
  if (!e || !ots.length) return;
  abrirDialogoEditor({
    titulo: 'Actividad de este día',
    campos: [{ id: 'ot', tipo: 'select', etiqueta: 'Actividad', valor: e.otNum, opciones: ots.map((o) => ({ valor: o.otNum, texto: etiquetaOtEditor(o) })) }],
    accionExtra: {
      texto: 'Quitar actividad del día',
      fn: async () => {
        if (!confirm('¿Quitar esta actividad (y sus pasos) de este día?')) return;
        await guardarPasosEntrada(e, []);
        await refrescarVistaInteractiva();
      },
    },
    onGuardar: async (v) => {
      const ot = ots.find((o) => String(o.otNum) === String(v.ot));
      await bitacoraCollection().doc(id).update({
        otNum: ot.otNum, otDescripcion: ot.descripcion, area: ot.area,
        titulo: ot.manual ? ot.descripcion : `OT ${ot.otNum} — ${ot.descripcion}`,
      });
      await refrescarVistaInteractiva();
...
function dialogoEditarTurno(ids) {
  const primera = entradaBitacoraPorId(ids[0]);
  if (!primera) return;
  abrirDialogoEditor({
    titulo: 'Turno',
    campos: [{ id: 'turno', tipo: 'select', etiqueta: 'Turno', valor: primera.turnoTipo, opciones: [{ valor: 'Día', texto: 'Día' }, { valor: 'Noche', texto: 'Noche' }] }],
    onGuardar: async (v) => {
      for (const id of ids) {
        const e = entradaBitacoraPorId(id);
        if (e) await bitacoraCollection().doc(id).update({ turnoTipo: v.turno, turnoIdx: turnoIdxDeFechaTurno(e.fecha, v.turno) });
      }
      await refrescarVistaInteractiva();
    },
  });
...
function dialogoEditarFechaDia(iso) {
  ...
  const entradas = state.bitacora.filter((b) => otsSet.has(String(b.otNum)) && b.fecha === iso && (b.bullets || []).length);
  if (!entradas.length) return;
  abrirDialogoEditor({
    titulo: 'Fecha del día',
    campos: [{ id: 'fecha', tipo: 'date', etiqueta: 'Fecha', valor: iso }],
    onGuardar: async (v) => {
      if (!v.fecha) return 'Elige la fecha';
      for (const e of entradas) await bitacoraCollection().doc(e.id).update({ fecha: v.fecha, turnoIdx: turnoIdxDeFechaTurno(v.fecha, e.turnoTipo) });
      await refrescarVistaInteractiva();
    },
  });
}

// ---- Conclusiones: una por cada actividad del informe (siempre van todas).
// Si no se escribe nada propio, se arma sola con el estado y los pasos reales;
// lo escrito a mano se guarda en informe.conclusiones[otNum]. ----
function conclusionActualDeOt(ot) {
  const propia = ((state.informeActivo.conclusiones || {})[ot.otNum] || '').trim();
...
function dialogoEditarConclusiones() {
  ...
  abrirDialogoEditor({
    titulo: 'Editar conclusiones',
    campos: ots.map((ot) => ({
      id: 'c_' + ot.otNum, tipo: 'textarea', filas: 4,
      etiqueta: `OT ${ot.tipo === 'Emergente' ? 'EMERGENTE' : ot.otNum} — ${tituloSinEquipo(ot.descripcion)}`,
      valor: conclusionActualDeOt(ot),
    })),
    accionExtra: {
      texto: 'Volver todas al texto automático',
      fn: async () => {
        await guardarCampoInformeActivo('conclusiones', {});
        await refrescarVistaInteractiva();
      },
    },
    onGuardar: async (v) => {
      const mapa = {};
      ots.forEach((ot) => {
        const texto = (v['c_' + ot.otNum] || '').trim();
        if (texto && texto !== generarConclusionTextoOt(ot).trim()) mapa[ot.otNum] = texto;
      });
      await guardarCampoInformeActivo('conclusiones', mapa);
      await refrescarVistaInteractiva();
    },
...
// ---- Componentes (los mismos de cada actividad en la app: state.componentes),
// herramientas y consumibles (texto del informe). "＋ Añadir componente" crea de
// inmediato una fila para la primera actividad; se toca el texto para editarlo. ----
async function anadirComponenteDirecto() {
  const ots = otsDelInforme(state.informeActivo);
  if (!ots.length) { showToast('Primero elige las actividades del informe'); return; }
  let nuevo = null;
  try {
    const ot = ots[0];
    nuevo = await componentesCollection().add({
      otNum: ot.otNum, otDescripcion: ot.descripcion, area: ot.area,
      codigoSAP: '', descripcion: '', cantidad: 1, createdAt: Date.now(),
    });
    editorDestacar = { comp: nuevo.id };
  } catch (e) { avisarErrorEditor(e, 'No se pudo añadir el componente'); }
  await refrescarVistaInteractiva();
  if (nuevo) dialogoEditarComponente(nuevo.id);
}

function dialogoEditarComponente(id) {
  const c = state.componentes.find((x) => x.id === id);
  const ots = otsDelInforme(state.informeActivo);
  if (!c) return;
  abrirDialogoEditor({
    titulo: 'Componente a retirar',
    campos: [
      { id: 'desc', tipo: 'text', etiqueta: 'Descripción', valor: c.descripcion || '' },
      { id: 'sap', tipo: 'text', etiqueta: 'Código SAP (opcional)', valor: c.codigoSAP || '' },
      { id: 'cant', tipo: 'number', etiqueta: 'Cantidad', valor: c.cantidad == null ? 1 : c.cantidad },
      { id: 'ot', tipo: 'select', etiqueta: 'Actividad', valor: c.otNum, opciones: ots.map((o) => ({ valor: o.otNum, texto: etiquetaOtEditor(o) })) },
    ],
    onGuardar: async (v) => {
      if (!v.desc.trim()) return 'Escribe la descripción del componente';
      const ot = ots.find((o) => String(o.otNum) === String(v.ot)) || ots[0];
      await componentesCollection().doc(id).update({
        descripcion: v.desc.trim(), codigoSAP: v.sap.trim(), cantidad: Number(v.cant) > 0 ? Number(v.cant) : 1,
        otNum: ot.otNum, otDescripcion: ot.descripcion, area: ot.area,
      });
      await refrescarVistaInteractiva();
    },
...
function dialogoEditarListaTexto(cual) {
  const informe = state.informeActivo;
  const esHerr = cual === 'herr';
  abrirDialogoEditor({
    titulo: esHerr ? 'Lista de herramientas' : 'Lista de consumibles',
    campos: [{
      id: 't', tipo: 'textarea', filas: 10,
      etiqueta: esHerr ? 'Una herramienta por línea — con cantidad: «Llave torquímetro 1" — 2» (o pega desde Excel: nombre ⇥ cantidad). Sin cantidad va 01.' : 'Un consumible por línea',
      valor: (esHerr ? informe.herramientasTexto : informe.consumiblesTexto) || '',
    }],
    onGuardar: async (v) => {
      await guardarCampoInformeActivo(esHerr ? 'herramientasTexto' : 'consumiblesTexto', v.t.trim());
      await refrescarVistaInteractiva();
    },
  });
}
...
function dialogoEditarRecomendacion(otNum) {
  ...
  abrirDialogoEditor({
    ...
    campos: [{ id: 't', tipo: 'textarea', etiqueta: 'Recomendación para esta actividad', filas: 5, valor: recs[otNum] || '' }],
    onGuardar: async (v) => {
      await guardarCampoInformeActivo('recomendaciones', { ...(state.informeActivo.recomendaciones || {}), [otNum]: v.t.trim() });
      await refrescarVistaInteractiva();
    },
  });
}

// Cambia a qué actividad pertenece una recomendación (entre las que aún no tienen).
function dialogoCambiarActividadRecomendacion(otNum) {
  const informe = state.informeActivo;
  const recs = informe.recomendaciones || {};
  const opciones = otsDelInforme(informe).filter((o) => String(o.otNum) === String(otNum) || !Object.prototype.hasOwnProperty.call(recs, o.otNum));
  abrirDialogoEditor({
    titulo: 'Actividad de la recomendación',
    campos: [{ id: 'ot', tipo: 'select', etiqueta: 'Actividad', valor: otNum, opciones: opciones.map((o) => ({ valor: o.otNum, texto: etiquetaOtEditor(o) })) }],
    onGuardar: async (v) => {
      if (String(v.ot) !== String(otNum)) {
        const nuevo = { ...recs };
        nuevo[v.ot] = nuevo[otNum] || '';
        delete nuevo[otNum];
        await guardarCampoInformeActivo('recomendaciones', nuevo);
      }
      await refrescarVistaInteractiva();
    },
  });
...
// ---- Filas de fotos ----
function parseRefFoto(ref) {
  if (ref.startsWith('e:')) {
    const [id, idx] = ref.slice(2).split('~');
    return { tipo: 'e', id, idx: Number(idx) };
  }
  return { tipo: 'p', idx: Number(ref.slice(2)) };
}

function fotoDeRef(ref) {
  const r = parseRefFoto(ref);
  if (r.tipo === 'e') { const e = entradaBitacoraPorId(r.id); return e ? (e.fotos || [])[r.idx] : null; }
  return (state.informeActivo.preparativosFotos || [])[r.idx] || null;
}

async function actualizarFotoDeRef(ref, cambios) {
  const r = parseRefFoto(ref);
  if (r.tipo === 'e') {
    const e = entradaBitacoraPorId(r.id);
    if (!e) return;
    await bitacoraCollection().doc(r.id).update({ fotos: (e.fotos || []).map((f, i) => (i === r.idx ? { ...f, ...cambios } : f)) });
  } else {
    await guardarCampoInformeActivo('preparativosFotos', (state.informeActivo.preparativosFotos || []).map((f, i) => (i === r.idx ? { ...f, ...cambios } : f)));
  }
}

async function anadirFilaFotosDirecto(tipo) {
  const informe = state.informeActivo;
  try {
    if (tipo === 'prep') {
      const lista = informe.preparativosFotos || [];
      const fecha = (lista.length && lista[lista.length - 1].fecha) || informe.preparativosFechaIni || hoyISOEditor();
      await guardarCampoInformeActivo('preparativosFotos', [...lista, { url: '', descripcion: '', fecha }, { url: '', descripcion: '', fecha }]);
      editorDestacar = { refs: `p:${lista.length}|` };
    } else {
      const ots = otsDelInforme(informe);
      if (!ots.length) { showToast('Primero elige las actividades del informe (toca la tabla de actividades)'); return; }
      const otsSet = new Set(ots.map((o) => String(o.otNum)));
      const conFotos = state.bitacora.filter((b) => otsSet.has(String(b.otNum)) && (b.fotos || []).length)
        .sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));
      const ult = conFotos[conFotos.length - 1];
      const ot = (ult && ots.find((o) => String(o.otNum) === String(ult.otNum))) || ots[0];
      const nueva = await bitacoraCollection().add({
        ...datosBaseEntradaBitacora(ot, ult ? ult.fecha : hoyISOEditor(), ult ? ult.turnoTipo : 'Día'),
        bullets: [], fotos: [{ url: '', descripcion: '' }, { url: '', descripcion: '' }],
      });
      editorDestacar = { refs: `e:${nueva.id}~0|` };
    }
  } catch (e) { avisarErrorEditor(e, 'No se pudo añadir la fila de fotos'); }
  await refrescarVistaInteractiva();
}

async function subirFotoDeEditor(file) {
  showToast('Subiendo foto…');
  return await subirArchivoAStorage(file, `editor_${Date.now()}_${file.name}`);
}

// Espacio que ya existe pero está vacío: se elige la foto y se guarda ahí.
async function fotoEnEspacio(ref) {
  const file = await elegirArchivoEditor('image/*');
  if (!file) return;
  try { await actualizarFotoDeRef(ref, { url: await subirFotoDeEditor(file) }); } catch (e) { avisarErrorEditor(e, 'No se pudo subir la foto'); }
  await refrescarVistaInteractiva();
}

// Segundo espacio libre de un par con una sola foto: se crea la foto nueva.
async function fotoNuevaEnGrupo(grupo) {
  const file = await elegirArchivoEditor('image/*');
  if (!file) return;
  try {
    const url = await subirFotoDeEditor(file);
    if (grupo.startsWith('p:')) {
      const idxA = Number(grupo.slice(2));
      const lista = [...(state.informeActivo.preparativosFotos || [])];
      lista.splice(idxA + 1, 0, { url, descripcion: '', fecha: (lista[idxA] && lista[idxA].fecha) || hoyISOEditor() });
      await guardarCampoInformeActivo('preparativosFotos', lista);
    } else {
      const e = entradaBitacoraPorId(grupo);
      if (e) await bitacoraCollection().doc(grupo).update({ fotos: [...(e.fotos || []), { url, descripcion: '' }] });
    }
  } catch (e) { avisarErrorEditor(e, 'No se pudo subir la foto'); }
  await refrescarVistaInteractiva();
}

// Quitar la foto deja el espacio (con su texto) para poner otra.
async function quitarFotoDeEspacio(ref) {
  try { await actualizarFotoDeRef(ref, { url: '' }); } catch (e) { avisarErrorEditor(e, 'No se pudo quitar la foto'); }
  await refrescarVistaInteractiva();
}

function dialogoEditarDescripcion(ref) {
  const foto = fotoDeRef(ref);
  if (!foto) return;
  abrirDialogoEditor({
    titulo: 'Descripción de la foto',
    campos: [{ id: 'd', tipo: 'textarea', etiqueta: 'Descripción', filas: 3, valor: foto.descripcion || '' }],
    onGuardar: async (v) => {
      await actualizarFotoDeRef(ref, { descripcion: v.d.trim() });
      await refrescarVistaInteractiva();
    },
  });
}

// Cambia la fecha de una fila de fotos. En preparativos es solo el dato de
// esas fotos; en parada las fotos pasan a la entrada de esa actividad y fecha.
function dialogoEditarFechaFila(refs) {
  const partes = refs.map(parseRefFoto);
  const primera = partes[0];
  let actual = hoyISOEditor();
  if (primera.tipo === 'p') actual = ((state.informeActivo.preparativosFotos || [])[primera.idx] || {}).fecha || actual;
  else { const e = entradaBitacoraPorId(primera.id); if (e) actual = e.fecha; }
  abrirDialogoEditor({
    titulo: 'Fecha de estas fotos',
    campos: [{ id: 'fecha', tipo: 'date', etiqueta: 'Fecha', valor: actual }],
    onGuardar: async (v) => {
      if (!v.fecha) return 'Elige la fecha';
      await moverFotosDeFila(partes, v.fecha);
      await refrescarVistaInteractiva();
    },
  });
}

async function moverFotosDeFila(partes, nuevaFecha) {
  if (partes[0].tipo === 'p') {
    const set = new Set(partes.map((p) => p.idx));
    await guardarCampoInformeActivo('preparativosFotos', (state.informeActivo.preparativosFotos || []).map((f, i) => (set.has(i) ? { ...f, fecha: nuevaFecha } : f)));
    return;
  }
  const porEntrada = {};
  partes.forEach((p) => { (porEntrada[p.id] = porEntrada[p.id] || []).push(p.idx); });
  for (const [id, idxs] of Object.entries(porEntrada)) {
    const e = entradaBitacoraPorId(id);
    if (!e || e.fecha === nuevaFecha) continue;
    const fotos = e.fotos || [];
    if (idxs.length === fotos.length && !(e.bullets || []).length) {
      await bitacoraCollection().doc(id).update({ fecha: nuevaFecha, turnoIdx: turnoIdxDeFechaTurno(nuevaFecha, e.turnoTipo) });
      continue;
    }
    const mover = idxs.map((i) => fotos[i]).filter(Boolean);
    const quedan = fotos.filter((_, i) => !idxs.includes(i));
    const destino = state.bitacora.find((b) => b.id !== id && String(b.otNum) === String(e.otNum) && b.fecha === nuevaFecha && b.turnoTipo === e.turnoTipo);
    if (destino) await bitacoraCollection().doc(destino.id).update({ fotos: [...(destino.fotos || []), ...mover] });
    else {
      await bitacoraCollection().add({
        otNum: e.otNum, otDescripcion: e.otDescripcion, area: e.area, titulo: e.titulo,
        fecha: nuevaFecha, turnoTipo: e.turnoTipo, turnoIdx: turnoIdxDeFechaTurno(nuevaFecha, e.turnoTipo),
        createdAt: Date.now(), bullets: [], fotos: mover,
      });
    }
    if (!quedan.length && !(e.bullets || []).length) await bitacoraCollection().doc(id).delete();
    else await bitacoraCollection().doc(id).update({ fotos: quedan });
  }
}

function dialogoAnadirAnexo(categoria) {
  const cat = ANEXOS_CATEGORIAS.find((c) => c.id === categoria);
  abrirDialogoEditor({
    titulo: `Añadir ${cat.etiqueta.toLowerCase()}`,
    campos: [
      { id: 'archivo', tipo: 'file', etiqueta: 'Imagen o PDF', accept: 'image/*,application/pdf' },
      { id: 'descripcion', tipo: 'text', etiqueta: `Descripción (sale en la leyenda: "${cat.etiqueta} N: …")` },
    ],
    onGuardar: async (v) => {
      if (!v.archivo) return 'Elige el archivo';
      const url = await subirArchivoAStorage(v.archivo, `anexos/${Date.now()}_${v.archivo.name}`);
      const nuevo = { nombre: v.archivo.name, url, tipo: v.archivo.type, categoria, descripcion: (v.descripcion || '').trim() };
      await guardarCampoInformeActivo('anexos', [...(state.informeActivo.anexos || []), nuevo]);
      await refrescarVistaInteractiva();
    },
  });
}

function dialogoAnadirDistribucion() {
  abrirDialogoEditor({
    titulo: 'Añadir imagen de distribución de población',
    campos: [{ id: 'archivo', tipo: 'file', etiqueta: 'Captura del cuadro de dotación (imagen)', accept: 'image/*' }],
    onGuardar: async (v) => {
      if (!v.archivo) return 'Elige la imagen';
      const url = await subirArchivoAStorage(v.archivo, `distribpoblacion_${Date.now()}_${v.archivo.name}`);
      await guardarUrlsDistribucionInforme([...urlsDistribucionInforme(state.informeActivo), url]);
      await refrescarVistaInteractiva();
    },
  });
}

function dialogoEditarPreparativos() {
  const informe = state.informeActivo;
  abrirDialogoEditor({
    titulo: 'Actividades de preparativos',
    campos: [
      { id: 'texto', tipo: 'textarea', etiqueta: 'Actividades (una por línea)', filas: 6, valor: (informe.preparativosTexto || []).join('\n') },
      { id: 'ini', tipo: 'date', etiqueta: 'Desde el', valor: informe.preparativosFechaIni || hoyISOEditor() },
      { id: 'fin', tipo: 'date', etiqueta: 'Hasta el (opcional)', valor: informe.preparativosFechaFin || '' },
    ],
    onGuardar: async (v) => {
      if (v.fin && v.fin < v.ini) return 'La fecha final no puede ser antes que la inicial';
      const lineas = (v.texto || '').split('\n').map((s) => s.trim()).filter(Boolean);
      await guardarCampoInformeActivo('preparativosTexto', lineas);
      await guardarCampoInformeActivo('preparativosFechaIni', v.ini);
      await guardarCampoInformeActivo('preparativosFechaFin', v.fin || '');
      await refrescarVistaInteractiva();
    },
  });
}

// ✕ de cada bloque: días, filas de fotos, anexos.
async function quitarDesdeEditor(sub, resto) {
  const informe = state.informeActivo;
  try {
    if (sub === 'comp') {
      if (!confirm('¿Quitar este componente? También desaparece de la lista de componentes de la actividad en la app.')) return;
      await componentesCollection().doc(resto).delete();
    } else if (sub === 'reco') {
      if (!confirm('¿Quitar esta recomendación?')) return;
      const nuevo = { ...(informe.recomendaciones || {}) };
      delete nuevo[resto];
      await guardarCampoInformeActivo('recomendaciones', nuevo);
    } else if (sub === 'distrib') {
      if (!confirm('¿Quitar esta imagen del informe?')) return;
      await guardarUrlsDistribucionInforme(urlsDistribucionInforme(informe).filter((_, i) => i !== Number(resto)));
    } else if (sub === 'anexo') {
      if (!confirm('¿Quitar este anexo del informe?')) return;
      await guardarCampoInformeActivo('anexos', (informe.anexos || []).filter((_, i) => i !== Number(resto)));
    } else if (sub === 'dia') {
      const otsSet = new Set((informe.otNums || []).map(String));
      const entradas = state.bitacora.filter((b) => otsSet.has(String(b.otNum)) && b.fecha === resto && (b.bullets || []).length);
      if (!confirm(`¿Quitar el día ${fechaDDMMYYYY(resto)}? Se borran sus pasos de la bitácora (también en el resto de la app); las fotos de esa fecha se conservan.`)) return;
      for (const e of entradas) await guardarPasosEntrada(e, []);
    } else if (sub === 'fotos') {
      if (!confirm('¿Quitar esta fila de fotos?')) return;
      const porEntrada = {};
      const prepIdx = [];
      resto.split('|').forEach((r) => {
        if (r.startsWith('e:')) {
          const [id, idx] = r.slice(2).split('~');
          if (!porEntrada[id]) porEntrada[id] = [];
          porEntrada[id].push(Number(idx));
        } else if (r.startsWith('p:')) prepIdx.push(Number(r.slice(2)));
      });
      for (const [id, idxs] of Object.entries(porEntrada)) {
        const e = state.bitacora.find((b) => b.id === id);
        if (!e) continue;
        const fotos = (e.fotos || []).filter((_, i) => !idxs.includes(i));
        if (!fotos.length && !(e.bullets || []).length) await bitacoraCollection().doc(id).delete();
        else await bitacoraCollection().doc(id).update({ fotos });
      }
      if (prepIdx.length) await guardarCampoInformeActivo('preparativosFotos', (informe.preparativosFotos || []).filter((_, i) => !prepIdx.includes(i)));
    }
  } catch (e) {
    console.error(e);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  safeInit(() => {
    const pantalla = document.getElementById('vistaInteractivaBackdrop');
    document.getElementById('vistaInteractivaCerrar').addEventListener('click', () => {
      pantalla.classList.remove('open');
      document.body.style.overflow = '';
      // Todo lo que se cambió en el documento ya está guardado en los mismos
      // datos que usa el formulario clásico: se vuelve a llenar con ellos para
    ...
    const conEstado = (idBtn, fn) => document.getElementById(idBtn).addEventListener('click', async () => {
      ...
      const txt = btn.textContent; btn.disabled = true; btn.textContent = 'Generando…';
      try { await fn(); } catch (e) { console.error(e); showToast(e.message || 'No se pudo generar el archivo'); }
      btn.disabled = false; btn.textContent = txt;
    });
    conEstado('editorBtnWord', async () => {
...
async function generatePdf() {
  ...
  descargarBlob(doc.output('blob'), fname);
}
...
async function generateComponentesReportPdf() {
  ...
  if (items.length === 0) {
    ...
    descargarBlob(doc.output('blob'), `Componentes_${PARADA_ID}.pdf`);
    return;
  }
  ...
  descargarBlob(doc.output('blob'), `Componentes_${PARADA_ID}_${new Date().toISOString().slice(0, 10)}.pdf`);
}
...
function renderComentarioFeed(otNum) {
  ...
  wrap.innerHTML = items.map((entry) => `
    <div class="comentario-entry">
      ...
      ${(entry.bullets || []).some((b) => b && b.trim()) ? `<ul class="comentario-entry-bullets">${entry.bullets.filter((b) => b && b.trim()).map((b) => `<li>${escBit(b)}</li>`).join('')}</ul>` : ''}
      ${entry.fotos && entry.fotos.length ? `<div class="comentario-entry-fotos">${entry.fotos.map((f) => `<div class="comentario-entry-foto">${f.url ? `<img src="${f.url}" loading="lazy">` : '<div style="border:1.5px dashed var(--line); border-radius:8px; padding:14px 6px; text-align:center; font-size:11px; color:var(--ink-dim);">(espacio de foto vacío)</div>'}<span>${escBit(f.descripcion || '')}</span></div>`).join('')}</div>` : ''}
      ${(!(entry.bullets || []).some((b) => b && b.trim()) && !(entry.fotos || []).length) ? '<div style="font-size:11.5px; color:var(--ink-dim);">(día creado en «Editar como documento» — pasos por escribir)</div>' : ''}
    </div>`).join('');
...
async function generateInformeActividadesPdf(supervisorFiltro) {
  ...
  descargarBlob(doc.output('blob'), `${nombreArchivo}_${new Date().toISOString().slice(0,10)}.pdf`);
}
...
async function generateInformeTurnoPdf() {
  ...
  drawFooter();
  descargarBlob(doc.output('blob'), `Informe_Por_Turno_${new Date().toISOString().slice(0, 10)}_${new Date().toTimeString().slice(0, 5).replace(':', '')}.pdf`);
}
...
async function generateInformeGeneralPdf() {
  ...
  drawFooter();
  descargarBlob(doc.output('blob'), `Informe_General_${new Date().toISOString().slice(0,10)}.pdf`);
}
